...
struct FluentSystemInfo {
    ...
    QMap<QString, QString>
        networkTypes;  // Interface -> Type (WiFi, Ethernet, etc.)
...
struct FluentSystemMetrics {
    ...
    std::chrono::milliseconds collectionTime{
        0};                // Time taken to collect metrics
...
public:
    ...
    FluentMonitoringLevel getMonitoringLevel() const {
        return m_config.monitoringLevel;
    }
    ...
    void enableResourceMonitoring(FluentSystemResource resource,
                                  bool enabled = true);
    ...
    void addAlertCallback(
        std::function<void(FluentSystemResource, const QString&)> callback);
    ...
    void exportSystemInfo(const QString& filePath,
...
private:
    ...
    QMap<int, std::function<void(FluentSystemResource, const QString&)>>
        m_alertCallbacks;
