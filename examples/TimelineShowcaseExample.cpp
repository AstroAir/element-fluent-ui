...
private slots:
    void onOrientationChanged() {
        auto orientation = m_orientationCombo->currentData()
                               .value<Components::FluentTimelineOrientation>();
        m_basicTimeline->setOrientation(orientation);
        m_projectTimeline->setOrientation(orientation);
    }

    void onAlignmentChanged() {
        auto alignment = m_alignmentCombo->currentData()
                             .value<Components::FluentTimelineAlignment>();
        m_basicTimeline->setAlignment(alignment);
        m_projectTimeline->setAlignment(alignment);
    }

    void onStyleChanged() {
        auto style = m_styleCombo->currentData()
                         .value<Components::FluentTimelineStyle>();
    ...
    void addRandomItem() {
        ...
        static QStringList titles = {"New Feature Implementation",
                                     "Bug Fix",
                                     "Code Review",
                                     "Testing Phase",
        ...
        QString title =
            titles[QRandomGenerator::global()->bounded(titles.size())];
        QString description = descriptions[QRandomGenerator::global()->bounded(
            descriptions.size())];
        ...
        item->setDateTime(
            QDateTime::currentDateTime().addSecs(itemCounter * 3600));

        // Random state
        auto states = {Components::FluentTimelineItemState::Pending,
        ...
        auto state = *std::next(
            states.begin(),
            QRandomGenerator::global()->bounded(qsizetype(states.size())));
    ...
    void clearItems() { m_interactiveTimeline->clearItems(); }

    void expandAllItems() { m_interactiveTimeline->expandAll(); }

    void collapseAllItems() { m_interactiveTimeline->collapseAll(); }

    void onTimelineItemClicked(Components::FluentTimelineItem* item,
                               int index) {
        m_statusLabel->setText(
            QString("Clicked: %1 (Index: %2)").arg(item->title()).arg(index));

        // Highlight clicked item temporarily
        QTimer::singleShot(2000, [this]() { m_statusLabel->setText("Ready"); });
    }
...
private:
    ...
    void setupControlsPanel(QWidget* parent) {
        ...
        m_orientationCombo->addItem(
            "Vertical", QVariant::fromValue(
                            Components::FluentTimelineOrientation::Vertical));
        m_orientationCombo->addItem(
            "Horizontal",
            QVariant::fromValue(
                Components::FluentTimelineOrientation::Horizontal));
        connect(m_orientationCombo,
                QOverload<int>::of(&QComboBox::currentIndexChanged), this,
                &TimelineShowcaseWindow::onOrientationChanged);
        configLayout->addWidget(m_orientationCombo);

        // Alignment
        configLayout->addWidget(new QLabel("Alignment:"));
        m_alignmentCombo = new QComboBox();
        m_alignmentCombo->addItem(
            "Left",
            QVariant::fromValue(Components::FluentTimelineAlignment::Left));
        m_alignmentCombo->addItem(
            "Right",
            QVariant::fromValue(Components::FluentTimelineAlignment::Right));
        m_alignmentCombo->addItem(
            "Center",
            QVariant::fromValue(Components::FluentTimelineAlignment::Center));
        m_alignmentCombo->addItem(
            "Alternate", QVariant::fromValue(
                             Components::FluentTimelineAlignment::Alternate));
        connect(m_alignmentCombo,
                QOverload<int>::of(&QComboBox::currentIndexChanged), this,
                &TimelineShowcaseWindow::onAlignmentChanged);
        ...
        m_styleCombo->addItem(
            "Default",
            QVariant::fromValue(Components::FluentTimelineStyle::Default));
        m_styleCombo->addItem(
            "Compact",
            QVariant::fromValue(Components::FluentTimelineStyle::Compact));
        m_styleCombo->addItem(
            "Detailed",
            QVariant::fromValue(Components::FluentTimelineStyle::Detailed));
        m_styleCombo->addItem(
            "Minimal",
            QVariant::fromValue(Components::FluentTimelineStyle::Minimal));
        connect(m_styleCombo,
                QOverload<int>::of(&QComboBox::currentIndexChanged), this,
                &TimelineShowcaseWindow::onStyleChanged);
        configLayout->addWidget(m_styleCombo);

        // Checkboxes
        m_animatedCheck = new QCheckBox("Animated");
        m_animatedCheck->setChecked(true);
        connect(m_animatedCheck, &QCheckBox::toggled, this,
                &TimelineShowcaseWindow::onAnimatedChanged);
        configLayout->addWidget(m_animatedCheck);

        m_scrollableCheck = new QCheckBox("Scrollable");
        connect(m_scrollableCheck, &QCheckBox::toggled, this,
                &TimelineShowcaseWindow::onScrollableChanged);
        configLayout->addWidget(m_scrollableCheck);

        m_connectorsCheck = new QCheckBox("Show Connectors");
        m_connectorsCheck->setChecked(true);
        connect(m_connectorsCheck, &QCheckBox::toggled, this,
                &TimelineShowcaseWindow::onShowConnectorsChanged);
    ...
    void setupBasicExample() {
        ...
        // Add basic items
        m_basicTimeline->addTextItem("Project Started",
                                     "Initial project setup and planning");
        m_basicTimeline->addTextItem("Development Phase",
                                     "Core functionality implementation");
        m_basicTimeline->addTextItem("Testing Phase",
                                     "Quality assurance and bug fixes");
        m_basicTimeline->addTextItem("Release",
    ...
    void setupProjectExample() {
        ...
        m_projectTimeline =
            Components::FluentTimeline::createDetailedTimeline(widget);
        ...
        auto* release = Components::FluentTimelineItem::createMilestone(
            "Release v1.0", QDateTime::fromString("2024-06-01", "yyyy-MM-dd"));
    ...
    void setupVariantsExample() {
        ...
        auto* compactTimeline =
            Components::FluentTimeline::createCompactTimeline();
        ...
        // Horizontal timeline
        auto* horizontalCard =
            new Components::FluentCard("Horizontal Timeline");
        auto* horizontalTimeline =
            Components::FluentTimeline::createHorizontalTimeline();
