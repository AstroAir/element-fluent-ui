...
public:
    AccessibilityPerformanceExample(QWidget* parent = nullptr)
        : QMainWindow(parent) {
        setupUI();
        setupAccessibility();
...
void AccessibilityPerformanceExample::setupUI() {
    ...
    Accessibility::setAccessibleDescription(
        this,
        "Demonstration of FluentQt accessibility features and performance "
        "monitoring");
...
QWidget* AccessibilityPerformanceExample::createAccessibilityPanel() {
    ...
    Accessibility::setAccessibleDescription(
        m_screenReaderCheck, "Enable enhanced screen reader support");
    ...
    Accessibility::setAccessibleDescription(
        m_animationsCheck, "Enable or disable animations for reduced motion");
    ...
    Accessibility::setAccessibleDescription(
        m_fontSizeSlider, "Adjust font size from 10 to 24 pixels");
    ...
    Accessibility::setAccessibleName(m_announceButton,
                                     "Test Screen Reader Announcement");
    Accessibility::setAccessibleDescription(
        m_announceButton, "Send a test message to screen readers");
    layout->addWidget(m_announceButton);

    // Reset button
    m_resetAccessibilityButton = new Components::FluentButton("Reset Settings");
    Accessibility::setAccessibleName(m_resetAccessibilityButton,
                                     "Reset Accessibility Settings");
    Accessibility::setAccessibleDescription(
        m_resetAccessibilityButton,
...
QWidget* AccessibilityPerformanceExample::createPerformancePanel() {
    ...
    Accessibility::setAccessibleDescription(
        m_performanceBar, "Shows current application performance level");
    layout->addWidget(m_performanceBar);

    // Performance test buttons
    m_performanceTestButton =
        new Components::FluentButton("Run Performance Test");
    Accessibility::setAccessibleName(m_performanceTestButton,
                                     "Run Performance Test");
    Accessibility::setAccessibleDescription(
        m_performanceTestButton, "Execute a comprehensive performance test");
    layout->addWidget(m_performanceTestButton);

    m_heavyOperationButton =
        new Components::FluentButton("Simulate Heavy Load");
    Accessibility::setAccessibleName(m_heavyOperationButton,
                                     "Simulate Heavy Load");
    Accessibility::setAccessibleDescription(
        m_heavyOperationButton, "Simulate a CPU-intensive operation");
    ...
    Accessibility::setAccessibleDescription(
        m_performanceLog, "List of performance events and warnings");
...
QWidget* AccessibilityPerformanceExample::createResponsivePanel() {
    ...
    Accessibility::setAccessibleName(m_responsiveCheck,
                                     "Responsive Mode Toggle");
    Accessibility::setAccessibleDescription(
        m_responsiveCheck, "Enable responsive layout adjustments");
    ...
    m_breakpointCombo->addItems({"Mobile (320px)", "Tablet (768px)",
                                 "Desktop (1024px)", "Large (1440px)"});
    m_breakpointCombo->setCurrentIndex(2);  // Desktop
    Accessibility::setAccessibleName(m_breakpointCombo, "Breakpoint Selector");
    Accessibility::setAccessibleDescription(
        m_breakpointCombo, "Select screen size breakpoint for testing");
    ...
    Accessibility::setAccessibleName(m_animationSpeedSlider,
                                     "Animation Speed Slider");
    Accessibility::setAccessibleDescription(
        m_animationSpeedSlider, "Adjust animation speed from 25% to 200%");
...
QWidget* AccessibilityPerformanceExample::createDemoContentPanel() {
    ...
    for (int i = 0; i < 12; ++i) {
        ...
        card->setSubtitle(
            QString("Accessibility and performance demo content"));
        card->setMinimumSize(200, 150);

        // Set up accessibility
        Accessibility::setAccessibleName(card,
                                         QString("Demo Card %1").arg(i + 1));
        Accessibility::setAccessibleDescription(
            card, QString("Interactive demo card number %1 for testing "
        ...
        auto* button =
            new Components::FluentButton(QString("Action %1").arg(i + 1), card);
        Accessibility::setAccessibleName(
            button, QString("Action Button %1").arg(i + 1));
        Accessibility::setAccessibleDescription(
            button,
            QString("Perform action %1 with accessibility support").arg(i + 1));

        auto* progress = new Components::FluentProgressBar(card);
        progress->setValue(QRandomGenerator::global()->bounded(20, 100));
        Accessibility::setAccessibleName(
            progress, QString("Progress Indicator %1").arg(i + 1));
        Accessibility::setAccessibleDescription(
            progress, QString("Shows progress for task %1").arg(i + 1));
...
void AccessibilityPerformanceExample::setupConnections() {
    ...
    connect(m_highContrastCheck, &Components::FluentCheckBox::toggled, this,
            &AccessibilityPerformanceExample::toggleHighContrast);
    connect(m_screenReaderCheck, &Components::FluentCheckBox::toggled, this,
            &AccessibilityPerformanceExample::toggleScreenReaderMode);
    connect(m_animationsCheck, &Components::FluentCheckBox::toggled, this,
            &AccessibilityPerformanceExample::toggleAnimations);
    connect(m_fontSizeSlider, &QSlider::valueChanged, this,
            &AccessibilityPerformanceExample::adjustFontSize);
    connect(m_announceButton, &Components::FluentButton::clicked, this,
            &AccessibilityPerformanceExample::announceToScreenReader);
    connect(m_resetAccessibilityButton, &Components::FluentButton::clicked,
            this, &AccessibilityPerformanceExample::resetAccessibilitySettings);

    // Performance monitoring
    connect(m_performanceMonitor,
            &Core::FluentPerformanceMonitor::frameRateChanged, this,
            &AccessibilityPerformanceExample::onFrameRateChanged);
    connect(m_performanceMonitor,
            &Core::FluentPerformanceMonitor::performanceWarning, this,
            &AccessibilityPerformanceExample::onPerformanceWarning);
    connect(m_performanceTestButton, &Components::FluentButton::clicked, this,
            &AccessibilityPerformanceExample::runPerformanceTest);
    connect(m_heavyOperationButton, &Components::FluentButton::clicked, this,
            &AccessibilityPerformanceExample::simulateHeavyOperation);
    connect(m_metricsTimer, &QTimer::timeout, this,
            &AccessibilityPerformanceExample::updatePerformanceMetrics);

    // Responsive design controls
    connect(m_responsiveCheck, &Components::FluentCheckBox::toggled, this,
            &AccessibilityPerformanceExample::toggleResponsiveMode);
    connect(
        m_breakpointCombo,
    ...
    connect(m_animationSpeedSlider, &QSlider::valueChanged, this,
            &AccessibilityPerformanceExample::adjustAnimationSpeed);

    // Demo button connections
    for (int i = 0; i < m_demoButtons.size(); ++i) {
        connect(m_demoButtons[i], &Components::FluentButton::clicked,
                [this, i]() {
                    profileOperation(
                        QString("Button Click %1").arg(i + 1), [this, i]() {
                            // Simulate some work
                            QThread::msleep(
                            ...
                            if (m_screenReaderMode) {
                                Accessibility::announceToScreenReader(
                                    QString("Action %1 completed").arg(i + 1));
                            }
                        });
                });
    }
...
void AccessibilityPerformanceExample::initializeDemo() {
    ...
    m_performanceLog->addItem(
        QString("[%1] Application started - Accessibility and Performance "
                "monitoring active")
...
void AccessibilityPerformanceExample::toggleHighContrast() {
    ...
    if (m_screenReaderMode) {
        QString message = m_highContrastMode ? "High contrast mode enabled"
    ...
    // Log the change
    m_performanceLog->addItem(
        QString("[%1] High contrast mode %2")
            .arg(QDateTime::currentDateTime().toString("hh:mm:ss"))
...
void AccessibilityPerformanceExample::adjustFontSize(int size) {
    ...
    if (m_screenReaderMode) {
        Accessibility::announceToScreenReader(
            QString("Font size changed to %1 pixels").arg(size));
    }
}
...
void AccessibilityPerformanceExample::toggleAnimations(bool enabled) {
    ...
    // Announce change to screen reader
    if (m_screenReaderMode) {
        QString message = enabled ? "Animations enabled"
...
void AccessibilityPerformanceExample::toggleScreenReaderMode(bool enabled) {
    ...
    if (enabled) {
        Accessibility::announceToScreenReader(
            "Screen reader mode enabled. Enhanced accessibility features are "
            "now active.");
    }

    // Log the change
    m_performanceLog->addItem(
        QString("[%1] Screen reader mode %2")
            .arg(QDateTime::currentDateTime().toString("hh:mm:ss"))
            .arg(enabled ? "enabled" : "disabled"));
}
...
void AccessibilityPerformanceExample::announceToScreenReader() {
    QString message =
        QString(
            "Accessibility test announcement at %1. Current settings: High "
            "contrast %2, Animations %3, Font size %4 pixels.")
            .arg(QDateTime::currentDateTime().toString("hh:mm:ss"))
            .arg(m_highContrastMode ? "enabled" : "disabled")
    ...
    m_performanceLog->addItem(
        QString("[%1] Screen reader test: %2")
            .arg(QDateTime::currentDateTime().toString("hh:mm:ss"))
            .arg(message));
}
...
void AccessibilityPerformanceExample::onFrameRateChanged(double fps) {
    ...
    m_performanceBar->setStyleSheet(
        QString("QProgressBar::chunk { background-color: %1; }").arg(color));
}

void AccessibilityPerformanceExample::onPerformanceWarning(
    ...
    if (m_screenReaderMode && message.contains("Very low")) {
        Accessibility::announceToScreenReader(
            "Performance warning: Very low frame rate detected");
    }
}
...
void AccessibilityPerformanceExample::runPerformanceTest() {
    ...
    m_performanceLog->addItem(
        QString("[%1] Performance test completed in %2ms")
            .arg(QDateTime::currentDateTime().toString("hh:mm:ss"))
            .arg(testTime.count()));
    ...
    if (m_screenReaderMode) {
        Accessibility::announceToScreenReader(
            QString("Performance test completed in %1 milliseconds")
                .arg(testTime.count()));
    }
}
...
void AccessibilityPerformanceExample::updatePerformanceMetrics() {
    ...
    m_memoryUsage = QRandomGenerator::global()->bounded(50, 200) * 1024 *
                    1024;  // 50-200 MB
    m_memoryLabel->setText(
        QString("Memory: %1 MB").arg(m_memoryUsage / (1024 * 1024)));
}
...
void AccessibilityPerformanceExample::simulateHeavyOperation() {
    ...
    auto operationTime =
        m_performanceMonitor->getProfileTime("Heavy Operation");
    m_performanceLog->addItem(
        QString("[%1] Heavy operation completed in %2ms")
            .arg(QDateTime::currentDateTime().toString("hh:mm:ss"))
...
void AccessibilityPerformanceExample::adjustAnimationSpeed(int speed) {
    ...
    m_performanceLog->addItem(
        QString("[%1] Animation speed set to %2%")
            .arg(QDateTime::currentDateTime().toString("hh:mm:ss"))
            .arg(speed));
}
...
void AccessibilityPerformanceExample::resetAccessibilitySettings() {
    ...
    m_performanceLog->addItem(
        QString("[%1] Accessibility settings reset to defaults")
            .arg(QDateTime::currentDateTime().toString("hh:mm:ss")));
...
void AccessibilityPerformanceExample::updatePerformanceDisplay() {
    ...
    if (m_performanceMonitor->getProfileTime("Button Click 1").count() != 0) {
        ...
        m_profileLabel->setText(
            QString("Last Operation: %1ms").arg(lastTime.count()));
    }
}
...
void AccessibilityPerformanceExample::simulateResponsiveBreakpoints() {
    ...
    if (m_screenReaderMode) {
        Accessibility::announceToScreenReader(
            QString("Layout changed to %1 breakpoint with %2 columns")
                .arg(m_currentBreakpoint)
                .arg(columns));
    }
}

void AccessibilityPerformanceExample::profileOperation(
