...
public:
    ...
    std::unique_ptr<QPropertyAnimation> dampingAnimation(
        QObject* target, const QByteArray& property, const QVariant& startValue,
        const QVariant& initialVelocity,
        const FluentDampingConfig& config = {});

    // Gesture-driven animations
    void startGestureTracking(QWidget* target,
                              const FluentGestureConfig& config = {});
    void updateGesturePosition(const QPointF& position,
...
public:
    enum GestureType { Pan, Pinch, Rotate, Swipe, Fling };

    explicit FluentGestureRecognizer(QWidget* target,
                                     QObject* parent = nullptr);
...
signals:
    ...
    void gestureUpdated(GestureType type, const QPointF& position,
                        const QVariant& data);
    void gestureFinished(GestureType type, const QPointF& position,
                         const QVariant& data);
