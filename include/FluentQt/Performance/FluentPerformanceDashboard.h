...
public:
    explicit FluentMetricCard(const QString& title, const QString& metric,
...
public:
    explicit FluentPerformanceChart(FluentDashboardWidgetType chartType,
                                    QWidget* parent = nullptr);
    ...
    void updateMetric(
        const QString& metricName, double value,
...
public:
    ...
    void updateComponent(const QString& componentName,
...
public:
    enum class AlertType { Info, Warning, Critical, Success };

    explicit FluentAlertWidget(AlertType type, const QString& message,
                               QWidget* parent = nullptr);

    void setMessage(const QString& message);
    void setType(AlertType type);
    void setAutoHide(bool autoHide, std::chrono::milliseconds duration =
...
public:
    ...
    FluentDashboardLayoutConfig getLayoutConfig() const {
        return m_layoutConfig;
    }
    void setUpdateInterval(std::chrono::milliseconds interval);

    // Widget management
    void addWidget(const QString& id, FluentDashboardWidgetType type,
                   const FluentDashboardWidgetConfig& config);
...
private:
    ...
    FluentMetricCard* createMetricCard(
        const FluentDashboardWidgetConfig& config);
    FluentPerformanceChart* createChart(
        const FluentDashboardWidgetConfig& config);
    FluentPerformanceTable* createTable(
        const FluentDashboardWidgetConfig& config);
    ...
    // Alert management
    void processAlert(FluentAlertWidget::AlertType type,
...
public:
    static FluentPerformanceDashboard* createOverviewDashboard(
        QWidget* parent = nullptr);
    static FluentPerformanceDashboard* createDetailedDashboard(
        QWidget* parent = nullptr);
    static FluentPerformanceDashboard* createDeveloperDashboard(
        QWidget* parent = nullptr);
    static FluentPerformanceDashboard* createMinimalDashboard(
        QWidget* parent = nullptr);
    static FluentPerformanceDashboard* createCustomDashboard(
    ...
    static FluentPerformanceChart* createPerformanceChart(
        QWidget* parent = nullptr);
    static FluentPerformanceTable* createComponentTable(
        QWidget* parent = nullptr);

    // Configuration helpers
    static FluentDashboardWidgetConfig createMetricCardConfig(
        const QString& title, const QString& metric);
    static FluentDashboardWidgetConfig createChartConfig(
        const QString& title, const QStringList& metrics);
    static FluentDashboardWidgetConfig createTableConfig(
        const QString& title, const QStringList& columns);
};
...
// Layout utilities
QSize calculateOptimalWidgetSize(const QSize& containerSize, int widgetCount,
                                 int columns);
QPoint calculateWidgetPosition(int index, int columns, const QSize& widgetSize,
                               int spacing);
int calculateOptimalColumns(const QSize& containerSize,
...
void exportDashboardAsHTML(FluentPerformanceDashboard* dashboard,
