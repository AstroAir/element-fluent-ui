...
private:
    void applyDesignTokens() {
        ...
        setStyleSheet(QString("QPushButton { border-radius: %1px; }").arg(borderRadius));
    }
    ...
    void applyVariantColors() {
        ...
        QColor textColor = FLUENT_COLOR(QString("button.%1.text").arg(variantStr));
    ...
    void setupHoverEffects() {
        ...
        connect(&FluentDesignTokenUtils::instance(), 
                &FluentDesignTokenUtils::tokenChanged,
                this, &TokenizedButton::onTokenChanged);
    }
...
private:
    void applyDesignTokens() {
        ...
        setStyleSheet(QString("QFrame { border-radius: %1px; }").arg(borderRadius));
    ...
    void setupLayout() {
        ...
        headerLabel->setStyleSheet(QString("color: %1;").arg(FLUENT_COLOR("color.text.primary").name()));
        layout->addWidget(headerLabel);
        
        // Card body
        auto* bodyLabel = new QLabel("This is the card content area. It uses design tokens for consistent styling.");
        bodyLabel->setFont(FLUENT_FONT("card.body.fontSize"));
        bodyLabel->setStyleSheet(QString("color: %1;").arg(FLUENT_COLOR("color.text.secondary").name()));
        ...
        auto* actionButton = new TokenizedButton("Action", TokenizedButton::Primary, TokenizedButton::Small);
...
private:
    void setupUI() {
        ...
        titleLabel->setStyleSheet(QString("color: %1; margin-bottom: %2px;")
    ...
    void setupColorPaletteDemo(QVBoxLayout* mainLayout) {
        ...
        for (int i = 0; i < 8; ++i) {
            ...
            colorLabel->setStyleSheet(QString("background-color: %1; color: white; padding: 8px; border-radius: 4px;")
                                     .arg(color.name()));
        ...
        for (int i = 0; i < 8; ++i) {
            ...
            colorLabel->setStyleSheet(QString("background-color: %1; color: %2; padding: 8px; border-radius: 4px;")
                                     .arg(color.name())
    ...
    void setupTypographyDemo(QVBoxLayout* mainLayout) {
        ...
        // Typography samples
        QStringList typoSamples = {
            "typography.display|Display Text",
        ...
        for (const QString& sample : typoSamples) {
            ...
            label->setStyleSheet(QString("color: %1; margin: 4px 0;")
                               .arg(FLUENT_COLOR("color.text.primary").name()));
    ...
    void setupComponentDemo(QVBoxLayout* mainLayout) {
        ...
        buttonLayout->addWidget(new TokenizedButton("Primary", TokenizedButton::Primary));
        buttonLayout->addWidget(new TokenizedButton("Secondary", TokenizedButton::Secondary));
        buttonLayout->addWidget(new TokenizedButton("Accent", TokenizedButton::Accent));
        buttonLayout->addWidget(new TokenizedButton("Subtle", TokenizedButton::Subtle));
        buttonLayout->addStretch();
        componentLayout->addLayout(buttonLayout);
        
        // Button sizes
        auto* sizeLayout = new QHBoxLayout();
        sizeLayout->addWidget(new TokenizedButton("Small", TokenizedButton::Primary, TokenizedButton::Small));
        sizeLayout->addWidget(new TokenizedButton("Medium", TokenizedButton::Primary, TokenizedButton::Medium));
        sizeLayout->addWidget(new TokenizedButton("Large", TokenizedButton::Primary, TokenizedButton::Large));
    ...
    void setupSpacingDemo(QVBoxLayout* mainLayout) {
        ...
        for (const QString& size : spacingSizes) {
            ...
            spacingDemo->setStyleSheet(QString("background-color: %1; margin: 2px 0;")
                                      .arg(FLUENT_BRAND_COLOR(120).name()));
            spacingDemo->setFixedHeight(spacing);
            
            auto* spacingLabel = new QLabel(QString("spacing.%1 = %2px").arg(size).arg(spacing));
    ...
    void setupTokenDemo() {
        ...
        connect(&FluentAdvancedThemeManager::instance(),
                &FluentAdvancedThemeManager::themeChanged,
                this, &DesignTokenExampleWindow::onThemeChanged);
        
        qDebug() << "Design token demo initialized";
        qDebug() << "Available tokens:" << FluentDesignTokenUtils::instance().getAvailableTokens().size();
    }
};
