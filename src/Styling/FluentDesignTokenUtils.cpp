    ...
    , m_themeManager(&FluentAdvancedThemeManager::instance())
{
    ...
    connect(m_themeManager, &FluentAdvancedThemeManager::themeChanged,
            this, &FluentDesignTokenUtils::onThemeChanged);
    connect(m_themeManager, &FluentAdvancedThemeManager::tokenUpdated,
            this, &FluentDesignTokenUtils::onTokenUpdated);
    
    // Initialize token cache
    initializeTokenCache();
}

QColor FluentDesignTokenUtils::getColor(const QString& tokenName, const QColor& fallback) const {
...
QColor FluentDesignTokenUtils::getNeutralColor(int shade) const {
    ...
    return getColor(tokenName, shade == 0 ? QColor("#ffffff") : QColor("#000000"));
}
...
QFont FluentDesignTokenUtils::getFont(const QString& tokenName, const QFont& fallback) const {
    QVariant value = resolveTokenWithFallback(tokenName, fallback);
...
QFont FluentDesignTokenUtils::getTitleFont(int level) const {
    QString tokenName;
    switch (level) {
    case 1: tokenName = "typography.title1"; break;
    case 2: tokenName = "typography.title2"; break;
    case 3: tokenName = "typography.title3"; break;
    default: tokenName = "typography.title1"; break;
    }
...
QFont FluentDesignTokenUtils::getBodyFont(bool strong) const {
    ...
    return getFont(tokenName, QFont("Segoe UI", 14, strong ? QFont::DemiBold : QFont::Normal));
}

QFont FluentDesignTokenUtils::getCaptionFont(int level) const {
    QString tokenName = level == 1 ? "typography.caption1" : "typography.caption2";
...
int FluentDesignTokenUtils::getComponentSpacing(const QString& component, const QString& property) const {
    QString tokenName = QString("spacing.%1.%2").arg(component).arg(property);
    return getSize(tokenName, 8);
}

QMargins FluentDesignTokenUtils::getComponentMargins(const QString& component, const QString& size) const {
...
int FluentDesignTokenUtils::getSize(const QString& tokenName, int fallback) const {
    QVariant value = resolveTokenWithFallback(tokenName, fallback);
    return value.toInt();
}

QSize FluentDesignTokenUtils::getComponentSize(const QString& component, const QString& size) const {
...
QString FluentDesignTokenUtils::getComponentShadow(const QString& component, const QString& state) const {
...
QHash<QString, QVariant> FluentDesignTokenUtils::getComponentTokens(const QString& component) const {
...
QVariant FluentDesignTokenUtils::getComponentToken(const QString& component, const QString& property,
                                                  const QString& state, const QString& size) const {
...
QStringList FluentDesignTokenUtils::getAvailableTokens(FluentTokenType type) const {
    return m_themeManager->getTokenNames(type);
}

QStringList FluentDesignTokenUtils::getComponentTokenNames(const QString& component) const {
...
QString FluentDesignTokenUtils::getTokenDescription(const QString& tokenName) const {
    // This would require extending the theme manager to store descriptions
    return QString("Design token: %1").arg(tokenName);
}

void FluentDesignTokenUtils::setCustomToken(const QString& tokenName, const QVariant& value, FluentTokenType type) {
...
QVariant FluentDesignTokenUtils::getResponsiveToken(const QString& tokenName, const QSize& viewportSize) const {
    ...
    QString responsiveToken = QString("%1.%2").arg(tokenName).arg(activeBreakpoint);
...
bool FluentDesignTokenUtils::isBreakpointActive(const QString& breakpoint, const QSize& viewportSize) const {
    int breakpointValue = getBreakpoint(breakpoint);
    return viewportSize.width() >= breakpointValue;
}

QVariant FluentDesignTokenUtils::getThemeAwareToken(const QString& tokenName, FluentThemeMode mode) const {
...
QColor FluentDesignTokenUtils::getAccessibleColor(const QString& tokenName, const QString& backgroundToken,
    ...
    return background.lightness() > 128 ? QColor(0, 0, 0) : QColor(255, 255, 255);
}

bool FluentDesignTokenUtils::validateColorContrast(const QString& foregroundToken, const QString& backgroundToken,
...
void FluentDesignTokenUtils::onTokenUpdated(const QString& tokenName, const QVariant& value) {
    updateTokenCache(tokenName, value);
    emit tokenChanged(tokenName, value);
}

QString FluentDesignTokenUtils::buildTokenName(const QString& category, const QString& property, 
...
QVariant FluentDesignTokenUtils::resolveTokenWithFallback(const QString& tokenName, const QVariant& fallback) const {
...
void FluentDesignTokenUtils::updateTokenCache(const QString& tokenName, const QVariant& value) {
    QMutexLocker locker(&m_cacheMutex);
    m_tokenCache[tokenName] = value;
}

double FluentDesignTokenUtils::calculateSimpleContrast(const QColor& color1, const QColor& color2) const {
