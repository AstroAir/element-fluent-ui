...
public:
    ...
    struct OptimizationConfig {
        ...
        bool enablePerceptualOptimization =
            true;                          // Enable perceptual optimizations
        ...
        std::chrono::milliseconds maxDuration{
            500};  // Maximum recommended duration
    };
     ...
     */
    static QEasingCurve optimizeCurve(const QEasingCurve& originalCurve,
                                      const OptimizationConfig& config = {});
    ...
    static std::chrono::milliseconds calculateOptimalDuration(
        double distance, CurveType curveType,
        const OptimizationConfig& config = {});

    /**
     * @brief Check if a curve is optimized for the given configuration
     */
    static bool isCurveOptimized(const QEasingCurve& curve,
                                 const OptimizationConfig& config = {});

    /**
     * @brief Get recommended curve for specific use cases
     */
    static CurveType getRecommendedCurve(const QString& useCase,
                                         const OptimizationConfig& config = {});
...
private:
    ...
    static QEasingCurve applyAccessibilityOptimization(
...
public:
    ...
    static std::chrono::milliseconds calculateFramePerfectDuration(
        std::chrono::milliseconds desiredDuration, double targetFPS = 60.0);
