    ...
    , m_memoryCheckTimer(new QTimer(this))
{
    ...
    connect(m_cleanupTimer, &QTimer::timeout, this, &FluentAdvancedMemoryManager::performCleanup);
    
    m_statsTimer->setInterval(1000); // 1 second
    m_statsTimer->setSingleShot(false);
    connect(m_statsTimer, &QTimer::timeout, this, &FluentAdvancedMemoryManager::updateStatistics);
    
    m_memoryCheckTimer->setInterval(5000); // 5 seconds
    m_memoryCheckTimer->setSingleShot(false);
    connect(m_memoryCheckTimer, &QTimer::timeout, this, &FluentAdvancedMemoryManager::checkMemoryLimit);
...
void FluentAdvancedMemoryManager::registerComponentType(const QString& typeName, 
    ...
    if (config.enablePreallocation && config.preallocationCount > 0) {
        ...
        qDebug() << "Pre-allocation requested for" << typeName << "count:" << config.preallocationCount;
    }
}

void FluentAdvancedMemoryManager::unregisterComponentType(const QString& typeName) {
    QMutexLocker locker(&m_mutex);
...
void FluentAdvancedMemoryManager::optimizeMemoryUsage() {
    ...
    for (auto it = m_componentCaches.begin(); it != m_componentCaches.end(); ++it) {
        const QString& typeName = it.key();
        auto& cache = it.value();
        
        if (!m_componentConfigs.contains(typeName)) continue;
...
void FluentAdvancedMemoryManager::cleanupUnusedMemory() {
    ...
    for (auto it = m_componentStats.begin(); it != m_componentStats.end();) {
        if (it->currentUsage == 0 && it->totalAllocated == it->totalDeallocated) {
...
FluentMemoryStats FluentAdvancedMemoryManager::getComponentStats(const QString& typeName) const {
    QMutexLocker locker(&m_mutex);
...
void FluentAdvancedMemoryManager::setGlobalStrategy(FluentMemoryStrategy strategy) {
    QMutexLocker locker(&m_mutex);
...
void FluentAdvancedMemoryManager::setCleanupInterval(std::chrono::milliseconds interval) {
...
void FluentAdvancedMemoryManager::trackAllocation(const QString& typeName, size_t size) {
...
void FluentAdvancedMemoryManager::trackDeallocation(const QString& typeName, size_t size) {
    m_globalStats.totalDeallocated += size;
    m_globalStats.currentUsage = (m_globalStats.currentUsage > size) ? 
                                 m_globalStats.currentUsage - size : 0;
    
    auto& componentStats = m_componentStats[typeName];
    componentStats.totalDeallocated += size;
    componentStats.currentUsage = (componentStats.currentUsage > size) ? 
                                  componentStats.currentUsage - size : 0;
}

void FluentAdvancedMemoryManager::updateAllocationRate() {
    auto now = std::chrono::steady_clock::now();
    auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(
        now - m_performanceTimer.elapsed()).count();
    
    if (elapsed > 0) {
        for (auto it = m_allocationCounts.begin(); it != m_allocationCounts.end(); ++it) {
            const QString& typeName = it.key();
...
FluentMemoryStrategy FluentAdvancedMemoryManager::determineOptimalStrategy(const QString& typeName) const {
