...
FluentPhysicsAnimator::FluentPhysicsAnimator(QObject* parent)
    : QObject(parent), m_physicsTimer(new QTimer(this)) {
    // Setup physics update timer
    m_physicsTimer->setInterval(PHYSICS_UPDATE_INTERVAL);
    connect(m_physicsTimer, &QTimer::timeout, this,
            &FluentPhysicsAnimator::updateSpringAnimation);

    // Connect to performance monitor for adaptive quality
    connect(
        &Core::FluentPerformanceMonitor::instance(),
        ...
        [this](double fps) {
            ...
            if (fps < 30) {
                ...
                m_physicsTimer->setInterval(
                    HIGH_PERF_UPDATE_INTERVAL);  // 120 FPS
            } else {
...
std::unique_ptr<QPropertyAnimation> FluentPhysicsAnimator::springAnimation(
    ...
    const QVariant& to, const FluentSpringConfig& config) {
    if (!target) {
        return nullptr;
    }
    ...
    auto animation =
        std::make_unique<FluentSpringAnimation>(target, property, config);
...
std::unique_ptr<QPropertyAnimation> FluentPhysicsAnimator::springScale(
    ...
    const QRect toGeometry(center.x() - toSize.width() / 2,
                           center.y() - toSize.height() / 2, toSize.width(),
                           toSize.height());

    auto animation =
        std::make_unique<FluentSpringAnimation>(target, "geometry", config);
...
std::unique_ptr<QPropertyAnimation> FluentPhysicsAnimator::springPosition(
    QWidget* target, const QPointF& from, const QPointF& to,
    const FluentSpringConfig& config) {
    if (!target) {
        return nullptr;
    }
    ...
    auto animation =
        std::make_unique<FluentSpringAnimation>(target, "pos", config);
...
std::unique_ptr<QSequentialAnimationGroup>
FluentPhysicsAnimator::sharedElementTransition(
    QWidget* fromElement, QWidget* toElement,
    ...
    const QRect fromGlobalRect =
        QRect(fromElement->mapToGlobal(QPoint(0, 0)), fromElement->size());
    const QRect toGlobalRect =
        QRect(toElement->mapToGlobal(QPoint(0, 0)), toElement->size());
    ...
    QGraphicsOpacityEffect* fromOpacity =
        new QGraphicsOpacityEffect(fromElement);
    ...
    auto morphAnim =
        new FluentSpringAnimation(transitionWidget, "geometry", springConfig);
    ...
    connect(group.get(), &QSequentialAnimationGroup::finished,
            [fromElement, toElement, transitionWidget]() {
                // Remove effects
...
std::unique_ptr<QParallelAnimationGroup> FluentPhysicsAnimator::morphTransition(
    ...
    auto geometryAnim =
        new FluentSpringAnimation(target, "geometry", springConfig);
...
QEasingCurve FluentPhysicsAnimator::createSpringEasing(
    const FluentSpringConfig& config) {
    ...
    curve.setCustomType([config](qreal progress) -> qreal {
        ...
        const double zeta =
            config.damping / (2.0 * qSqrt(config.stiffness * config.mass));
...
QEasingCurve FluentPhysicsAnimator::createDampingEasing(
...
void FluentPhysicsAnimator::updateSpringAnimation() {
    ...
    QPointF force = calculateSpringForce(
        m_springState.position, m_springState.target, m_springState.config);
    ...
    QVector2D displacement =
        QVector2D(m_springState.target - m_springState.position);
...
QPointF FluentPhysicsAnimator::calculateSpringForce(
    const QPointF& position, const QPointF& target,
...
FluentSpringAnimation::FluentSpringAnimation(QObject* target,
...
QVariant FluentSpringAnimation::interpolated(const QVariant& from,
                                             const QVariant& to,
                                             qreal progress) const {
    // For most properties, use the default interpolation with spring easing
    if (from.metaType().id() != QMetaType::QPointF &&
        from.metaType().id() != QMetaType::QPoint) {
        return QPropertyAnimation::interpolated(from, to, progress);
    }

    // Custom interpolation for position-based properties with velocity
    // consideration
    auto now = std::chrono::steady_clock::now();
    auto deltaTime =
    ...
    const double zeta =
        m_config.damping / (2.0 * qSqrt(m_config.stiffness * m_config.mass));
...
FluentGestureRecognizer::FluentGestureRecognizer(QWidget* target,
                                                 QObject* parent)
...
void FluentGestureRecognizer::setGestureEnabled(GestureType type,
                                                bool enabled) {
...
void FluentGestureRecognizer::setSensitivity(GestureType type,
                                             double sensitivity) {
    ...
    switch (event->type()) {
            ...
            if (m_gestureStates[Pan].active) {
                auto deltaTime =
                    std::chrono::duration_cast<std::chrono::milliseconds>(
                        now - m_gestureStates[Pan].lastTime)
                        .count();

                if (deltaTime > 0) {
                    QVector2D velocity =
                        QVector2D(position -
                                  m_gestureStates[Pan].lastPosition) /
                        deltaTime * 1000.0;
                    ...
                    emit gestureUpdated(Pan, position,
                                        QVariant::fromValue(velocity));
                }
            }
            break;
        ...
        case QEvent::MouseButtonRelease:
            if (m_gestureStates[Pan].active) {
                auto totalTime =
                    std::chrono::duration_cast<std::chrono::milliseconds>(
                        now - m_gestureStates[Pan].startTime)
                        .count();

                QVector2D totalDisplacement =
                    QVector2D(position - m_gestureStates[Pan].startPosition);
                ...
                if (velocity.length() > 500.0 && isGestureEnabled(Fling)) {
                    ...
                    emit gestureFinished(Fling, position,
                                         QVariant::fromValue(velocity));
                } else if (totalDisplacement.length() > 50.0 &&
                           isGestureEnabled(Swipe)) {
                    emit gestureStarted(Swipe, position);
                    emit gestureFinished(
                        Swipe, position,
                        QVariant::fromValue(totalDisplacement));
                }

                m_gestureStates[Pan].active = false;
                emit gestureFinished(Pan, position,
                                     QVariant::fromValue(velocity));
            }
            break;
...
void FluentGestureRecognizer::processTouchEvent(QTouchEvent* event) {
    ...
    if (touchPoints.size() == 2 &&
        (isGestureEnabled(Pinch) || isGestureEnabled(Rotate))) {
        ...
        if (event->type() == QEvent::TouchBegin) {
            if (isGestureEnabled(Pinch)) {
                double distance =
                    QVector2D(point2.position() - point1.position()).length();
            ...
            if (isGestureEnabled(Rotate)) {
                QVector2D vector =
                    QVector2D(point2.position() - point1.position());
            ...
            if (isGestureEnabled(Pinch)) {
                double distance =
                    QVector2D(point2.position() - point1.position()).length();
                double scale =
                    distance / m_gestureStates[Pinch].customData.toDouble();
            ...
            if (isGestureEnabled(Rotate)) {
                ...
                double angle = qAtan2(vector.y(), vector.x());
                double rotation =
                    angle - m_gestureStates[Rotate].customData.toDouble();
...
void FluentGestureRecognizer::processWheelEvent(QWheelEvent* event) {
    ...
    if (isGestureEnabled(Pan)) {
        ...
        QVector2D delta =
            QVector2D(event->angleDelta().x(), event->angleDelta().y()) / 120.0;
