...
public:
    explicit AnimationDemoCard(const QString& title, const QString& description,
...
public slots:
    void startAnimation() {
        if (m_currentAnimation &&
            m_currentAnimation->state() == QAbstractAnimation::Running) {
            return;
        }
        ...
        scaleAnimation->setStartValue(
            QRect(geometry().x(), geometry().y(), 0, 0));
    ...
    void setAccessibilityMode(
        Animation::FluentAccessibilityAnimationMode mode) {
...
private:
    void setupUI() {
        ...
        connect(m_actionButton, &QPushButton::clicked, this,
                &AnimationDemoCard::startAnimation);
    ...
    void setupAnimations() {
        // Initialize animation parameters based on current style and
        // accessibility mode
        updateAnimationParameters();
    }
    ...
    void setupGestures() {
        ...
        m_gestureController =
            new Animation::FluentGestureAnimationController(this, this);
        m_gestureController->enableGesture(Animation::FluentGestureType::Tap,
                                           true);
        m_gestureController->enableGesture(Animation::FluentGestureType::Hover,
                                           true);
        m_gestureController->enableGesture(Animation::FluentGestureType::Pan,
                                           true);

        // Configure gesture feedback
        m_gestureController->setGestureFeedback(
            Animation::FluentGestureType::Tap,
            Animation::FluentGestureFeedback::Visual);
        m_gestureController->setGestureFeedback(
            Animation::FluentGestureType::Hover,
            Animation::FluentGestureFeedback::Visual);
    }
    ...
    void startHoverEffect() {
        ...
        // Animate shadow
        auto* shadowAnimation =
            new QPropertyAnimation(shadowEffect, "blurRadius");
    ...
    void createRippleEffect(const QPoint& center) {
        ...
        ripple->setStyleSheet(
            "background-color: rgba(255, 255, 255, 0.3); border-radius: 0px;");
        ripple->show();
        ...
        connect(group, &QParallelAnimationGroup::finished, ripple,
                &QWidget::deleteLater);
        group->start(QAbstractAnimation::DeleteWhenStopped);
    }
...
private:
    ...
    Animation::FluentCulturalAnimationStyle m_animationStyle{
        Animation::FluentCulturalAnimationStyle::Western};
    Animation::FluentAccessibilityAnimationMode m_accessibilityMode{
        Animation::FluentAccessibilityAnimationMode::Full};
...
private slots:
    void updateMetrics() {
        auto& performanceManager =
            Animation::FluentAnimationPerformanceManager::instance();
        auto metrics = performanceManager.getCurrentMetrics();

        m_fpsLabel->setText(
            QString("FPS: %1").arg(metrics.averageFPS, 0, 'f', 1));
        m_frameTimeLabel->setText(
            QString("Frame Time: %1ms").arg(metrics.frameTime, 0, 'f', 2));
        m_activeAnimationsLabel->setText(
            QString("Active: %1").arg(metrics.activeAnimations));
        m_performanceScoreLabel->setText(
            QString("Score: %1").arg(metrics.performanceScore, 0, 'f', 1));

        // Update progress bars
        m_fpsProgressBar->setValue(static_cast<int>(metrics.averageFPS));
        m_performanceProgressBar->setValue(
            static_cast<int>(metrics.performanceScore));
...
public:
    explicit AnimationControlPanel(QWidget* parent = nullptr)
        : QWidget(parent) {
        setupUI();
        connectSignals();
    }
...
private slots:
    void onStyleChanged() {
        auto style = static_cast<Animation::FluentCulturalAnimationStyle>(
            m_styleCombo->currentData().toInt());
        emit animationStyleChanged(style);
    }

    void onAccessibilityChanged() {
        auto mode = static_cast<Animation::FluentAccessibilityAnimationMode>(
            m_accessibilityCombo->currentData().toInt());
        emit accessibilityModeChanged(mode);
    }

    void onPerformanceChanged() {
        auto level = static_cast<Animation::FluentAnimationPerformance>(
            m_performanceCombo->currentData().toInt());
...
private:
    void setupUI() {
        ...
        m_styleCombo->addItem(
            "Western",
            static_cast<int>(Animation::FluentCulturalAnimationStyle::Western));
        m_styleCombo->addItem(
            "Eastern",
            static_cast<int>(Animation::FluentCulturalAnimationStyle::Eastern));
        m_styleCombo->addItem(
            "Minimal",
            static_cast<int>(Animation::FluentCulturalAnimationStyle::Minimal));
        m_styleCombo->addItem(
            "Expressive",
            static_cast<int>(
                Animation::FluentCulturalAnimationStyle::Expressive));
        m_styleCombo->addItem(
            "Formal",
            static_cast<int>(Animation::FluentCulturalAnimationStyle::Formal));
        m_styleCombo->addItem(
            "Playful",
            static_cast<int>(Animation::FluentCulturalAnimationStyle::Playful));
        ...
        m_accessibilityCombo->addItem(
            "Full", static_cast<int>(
                        Animation::FluentAccessibilityAnimationMode::Full));
        m_accessibilityCombo->addItem(
            "Reduced",
            static_cast<int>(
                Animation::FluentAccessibilityAnimationMode::Reduced));
        m_accessibilityCombo->addItem(
            "Essential",
            static_cast<int>(
                Animation::FluentAccessibilityAnimationMode::Essential));
        m_accessibilityCombo->addItem(
            "None", static_cast<int>(
                        Animation::FluentAccessibilityAnimationMode::None));
        ...
        m_performanceCombo->addItem(
            "High",
            static_cast<int>(Animation::FluentAnimationPerformance::High));
        m_performanceCombo->addItem(
            "Medium",
            static_cast<int>(Animation::FluentAnimationPerformance::Medium));
        m_performanceCombo->addItem(
            "Low",
            static_cast<int>(Animation::FluentAnimationPerformance::Low));
        m_performanceCombo->addItem(
            "Adaptive",
            static_cast<int>(Animation::FluentAnimationPerformance::Adaptive));
        m_performanceCombo->addItem(
            "Power Saver",
            static_cast<int>(
                Animation::FluentAnimationPerformance::PowerSaver));
    ...
    void connectSignals() {
        connect(m_styleCombo,
                ...
                &AnimationControlPanel::onStyleChanged);
        connect(m_accessibilityCombo,
                QOverload<int>::of(&QComboBox::currentIndexChanged), this,
                &AnimationControlPanel::onAccessibilityChanged);
        connect(m_performanceCombo,
                QOverload<int>::of(&QComboBox::currentIndexChanged), this,
                &AnimationControlPanel::onPerformanceChanged);
    }
...
public:
    explicit AdvancedAnimationMainWindow(QWidget* parent = nullptr)
        : QMainWindow(parent) {
        setupUI();
        connectSignals();
...
private slots:
    void onAnimationStyleChanged(
        Animation::FluentCulturalAnimationStyle style) {
...
private:
    ...
    void connectSignals() {
        ...
        connect(m_controlPanel,
                &AnimationControlPanel::accessibilityModeChanged, this,
                &AdvancedAnimationMainWindow::onAccessibilityModeChanged);
    }

    void createAnimationCards() {
        QStringList titles = {"Entrance Animation", "Gesture Response",
                              "Physics Simulation", "Cultural Adaptation",
                              "Accessibility Mode", "Performance Optimization",
                              "Easing Curves",      "Color Interpolation",
                              "Transform Animation"};

        QStringList descriptions = {"Smooth entrance with scale and opacity",
        ...
        for (int i = 0; i < titles.size(); ++i) {
            auto* card =
                new AnimationDemoCard(titles[i], descriptions[i], this);
