...
public:
    explicit FluentAsyncValidator(const QString& name,
    ...
    void setAsyncValidator(
        std::function<void(const QVariant&,
                           std::function<void(FluentValidationResult)>)>
            validator);
...
public:
    // Basic validators
    static std::unique_ptr<FluentValidatorBase> required(
        const QString& message = "This field is required");
    static std::unique_ptr<FluentValidatorBase> minLength(
        int length, const QString& message = "");
    static std::unique_ptr<FluentValidatorBase> maxLength(
        int length, const QString& message = "");
    static std::unique_ptr<FluentValidatorBase> exactLength(
        int length, const QString& message = "");

    // Numeric validators
    static std::unique_ptr<FluentValidatorBase> minValue(
        double min, const QString& message = "");
    static std::unique_ptr<FluentValidatorBase> maxValue(
        double max, const QString& message = "");
    static std::unique_ptr<FluentValidatorBase> range(
        double min, double max, const QString& message = "");
    static std::unique_ptr<FluentValidatorBase> integer(
        const QString& message = "Must be an integer");
    static std::unique_ptr<FluentValidatorBase> decimal(
        int places = 2, const QString& message = "");

    // Pattern validators
    static std::unique_ptr<FluentValidatorBase> regex(
        const QRegularExpression& pattern, const QString& message = "");
    static std::unique_ptr<FluentValidatorBase> email(
        const QString& message = "Invalid email address");
    static std::unique_ptr<FluentValidatorBase> url(
        const QString& message = "Invalid URL");
    static std::unique_ptr<FluentValidatorBase> phone(
        const QString& message = "Invalid phone number");
    static std::unique_ptr<FluentValidatorBase> creditCard(
        const QString& message = "Invalid credit card number");

    // Date/time validators
    static std::unique_ptr<FluentValidatorBase> date(
        const QString& format = "yyyy-MM-dd", const QString& message = "");
    static std::unique_ptr<FluentValidatorBase> time(
        const QString& format = "hh:mm:ss", const QString& message = "");
    static std::unique_ptr<FluentValidatorBase> dateRange(
        const QDate& min, const QDate& max, const QString& message = "");
    static std::unique_ptr<FluentValidatorBase> futureDate(
        const QString& message = "Date must be in the future");
    static std::unique_ptr<FluentValidatorBase> pastDate(
        const QString& message = "Date must be in the past");

    // File validators
    static std::unique_ptr<FluentValidatorBase> fileSize(
        qint64 maxSize, const QString& message = "");
    static std::unique_ptr<FluentValidatorBase> fileExtension(
        const QStringList& extensions, const QString& message = "");
    static std::unique_ptr<FluentValidatorBase> imageFile(
        const QString& message = "Must be a valid image file");

    // Custom validators
    static std::unique_ptr<FluentValidatorBase> custom(
        std::function<FluentValidationResult(const QVariant&)> validator,
        const QString& name = "Custom");

    // Async validators
    static std::unique_ptr<FluentAsyncValidator> asyncEmail(
        const QString& message = "Email address is not available");
    static std::unique_ptr<FluentAsyncValidator> asyncUsername(
        const QString& message = "Username is not available");
    static std::unique_ptr<FluentAsyncValidator> asyncUrl(
        const QString& message = "URL is not accessible");
};
...
public:
    ...
    // Add validators
    FluentValidationRule& addValidator(
        std::unique_ptr<FluentValidatorBase> validator);
    ...
    FluentValidationRule& regex(const QRegularExpression& pattern,
    ...
    void setSeverity(FluentValidationSeverity severity) {
        m_severity = severity;
    }
...
public:
    ...
    // Field validation
    QList<FluentValidationResult> validateField(const QString& fieldName,
    ...
    void addCrossFieldRule(
        const QString& ruleName, const QStringList& fieldNames,
    ...
    void setFieldTrigger(const QString& fieldName,
                         FluentValidationTrigger trigger);

    // Debounced validation
    void setDebounceDelay(int milliseconds) { m_debounceDelay = milliseconds; }
    void validateFieldDebounced(const QString& fieldName,
                                const QVariant& value);
...
signals:
    void fieldValidated(const QString& fieldName,
                        const QList<FluentValidationResult>& results);
    void formValidated(const QList<FluentValidationResult>& results);
    void validationStateChanged(bool isValid);
    void fieldErrorsChanged(const QString& fieldName,
...
private:
    ...
    std::unordered_map<
        QString, std::function<FluentValidationResult(const QVariantMap&)>>
        m_crossFieldRules;
...
public:
    ...
    // Widget registration
    void registerWidget(const QString& fieldName, QWidget* widget,
                        const FluentValidationRule& rule);
    void unregisterWidget(const QString& fieldName);
    void unregisterWidget(QWidget* widget);

    // Automatic validation setup
    void setupAutoValidation(
        QWidget* widget,
        FluentValidationTrigger trigger = FluentValidationTrigger::OnChange);
    ...
    void showValidationFeedback(QWidget* widget,
                                const FluentValidationResult& result);
...
private:
    ...
    void applyValidationStyle(QWidget* widget,
                              const FluentValidationResult& result);
