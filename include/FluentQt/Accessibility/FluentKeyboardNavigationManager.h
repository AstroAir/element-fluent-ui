...
public:
    ...
    QWidget* getNextFocusWidget(QWidget* current,
    ...
    void registerShortcut(const QKeySequence& sequence,
                          ...
                          FluentShortcutType type = FluentShortcutType::Global);
    ...
    QList<FluentKeyboardShortcut> getShortcuts(
        FluentShortcutType type = FluentShortcutType::Global) const;
    ...
    QWidget* findSpatialNeighbor(QWidget* current,
                                 FluentFocusDirection direction);
    ...
    void announceShortcut(const QKeySequence& sequence,
                          const QString& description);
...
signals:
    ...
    void shortcutActivated(const QKeySequence& sequence,
                           const QString& description);
    ...
    void focusIndicatorChanged(QWidget* widget,
                               FluentFocusIndicatorStyle style);
...
private:
    ...
    double calculateDistance(const QRect& from, const QRect& to,
                             FluentFocusDirection direction);
    QWidget* findBestSpatialMatch(QWidget* current,
...
private:
    ...
    std::unordered_map<QWidget*, std::pair<int, int>>
        m_spatialGrids;  // rows, columns

    // Roving tabindex
    std::unordered_map<QWidget*, QList<QWidget*>> m_rovingGroups;
    std::unordered_map<QWidget*, QWidget*>
        m_rovingActive;  // container -> active widget
