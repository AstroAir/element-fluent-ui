...
private:
    void setupUI() {
        ...
        // Title
        auto* titleLabel =
            new QLabel("Fluent UI Feedback Components Demo", this);
        auto& theme = Styling::FluentTheme::instance();
    ...
    void setupProgressBarSection(QVBoxLayout* mainLayout) {
        ...
        m_determinateProgress = new Components::FluentProgressBar(
            Components::FluentProgressBarType::Determinate, this);
        ...
        auto* indeterminateProgress = new Components::FluentProgressBar(
            Components::FluentProgressBarType::Indeterminate, this);
        indeterminateProgress->start();
        typesLayout->addWidget(indeterminateProgress, 1, 1);

        // Ring
        typesLayout->addWidget(new QLabel("Ring:", this), 2, 0);
        m_ringProgress = new Components::FluentProgressBar(
            Components::FluentProgressBarType::Ring, this);
        m_ringProgress->setValue(75);
        typesLayout->addWidget(m_ringProgress, 2, 1);

        // Dots
        typesLayout->addWidget(new QLabel("Dots:", this), 3, 0);
        auto* dotsProgress = new Components::FluentProgressBar(
            Components::FluentProgressBarType::Dots, this);
        dotsProgress->start();
    ...
    void setupLoadingIndicatorSection(QVBoxLayout* mainLayout) {
        ...
        const QStringList typeNames = {"Spinner", "Dots", "Pulse",
                                       "Bars",    "Ring", "Wave"};
        ...
        for (int i = 0; i < loadingTypes.size(); ++i) {
            ...
            typesLayout->addWidget(new QLabel(typeNames[i] + ":", this), row,
                                   col);

            auto* indicator =
                new Components::FluentLoadingIndicator(loadingTypes[i], this);
            indicator->start();
    ...
    void setupControls(QVBoxLayout* mainLayout) {
        ...
        m_progressTypeCombo->addItems(
            {"Determinate", "Indeterminate", "Ring", "Dots"});
    ...
    void setupConnections() {
        connect(m_progressSlider, &QSlider::valueChanged, this,
                &FeedbackComponentsWindow::onProgressValueChanged);
        connect(m_progressTypeCombo,
                QOverload<int>::of(&QComboBox::currentIndexChanged), this,
                &FeedbackComponentsWindow::onProgressTypeChanged);
        connect(m_loadingTypeCombo,
                QOverload<int>::of(&QComboBox::currentIndexChanged), this,
                &FeedbackComponentsWindow::onLoadingTypeChanged);
        connect(m_loadingSizeCombo,
                QOverload<int>::of(&QComboBox::currentIndexChanged), this,
                &FeedbackComponentsWindow::onLoadingSizeChanged);
        connect(m_toggleLoadingButton, &QPushButton::clicked, this,
                &FeedbackComponentsWindow::onToggleLoading);
        ...
        if (themeButton && themeButton->text() == "Toggle Theme") {
            connect(themeButton, &QPushButton::clicked, this,
                    &FeedbackComponentsWindow::onThemeToggle);
        }
    }
    ...
    void setupDemoTimer() {
        ...
        connect(m_demoTimer, &QTimer::timeout, this,
                &FeedbackComponentsWindow::onDemoStep);
        m_demoTimer->start();
    }
