...
void FluentCrossPlatformTest::cleanupTestCase() { delete m_testWidget; }
...
void FluentCrossPlatformTest::testOperatingSystemVersion() {
    ...
    qDebug() << "OS Version:" << osVersion.name() << osVersion.majorVersion()
             << osVersion.minorVersion() << osVersion.microVersion();
...
void FluentCrossPlatformTest::testFontRendering() {
    ...
    qDebug() << "Button font metrics - Width:" << textWidth
             << "Height:" << textHeight;
}
...
void FluentCrossPlatformTest::testSystemThemeDetection() {
    ...
    qDebug() << "  WindowText:"
             << systemPalette.color(QPalette::WindowText).name();
    ...
    QVERIFY(FluentTheme::currentTheme() !=
            FluentTheme::System);  // Should be set to specific theme
}
...
void FluentCrossPlatformTest::testKeyboardLayouts() {
    ...
    qDebug() << "Language:"
             << QLocale::languageToString(systemLocale.language());
...
void FluentCrossPlatformTest::testPlatformPerformance() {
    ...
    qDebug() << "  Creation time:" << creationTime << "ms for" << componentCount
             << "components";
    qDebug() << "  Render time:" << renderTime << "ms";

    // Performance assertions (platform-dependent thresholds)
    QVERIFY2(creationTime < 5000,
             "Component creation should be reasonably fast");
...
void FluentCrossPlatformTest::testComponentOnPlatform(QWidget* component,
