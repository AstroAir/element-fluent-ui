...
public:
    ...
    QColor getColor(const QString& tokenName, const QColor& fallback = QColor()) const;
    ...
    QFont getFont(const QString& tokenName, const QFont& fallback = QFont()) const;
    ...
    int getComponentSpacing(const QString& component, const QString& property) const;
    QMargins getComponentMargins(const QString& component, const QString& size = "medium") const;
    
    // Size token utilities
    int getSize(const QString& tokenName, int fallback = 0) const;
    QSize getComponentSize(const QString& component, const QString& size = "medium") const;
    ...
    QString getComponentShadow(const QString& component, const QString& state = "rest") const;
    ...
    QVariant getComponentToken(const QString& component, const QString& property, 
    ...
    QStringList getAvailableTokens(FluentTokenType type = FluentTokenType::Custom) const;
    ...
    QVariant getResponsiveToken(const QString& tokenName, const QSize& viewportSize) const;
    int getBreakpoint(const QString& size) const;
    bool isBreakpointActive(const QString& breakpoint, const QSize& viewportSize) const;
    
    // Theme-aware token utilities
    QVariant getThemeAwareToken(const QString& tokenName, FluentThemeMode mode) const;
    void invalidateTokenCache();
    
    // Accessibility-aware token utilities
    QColor getAccessibleColor(const QString& tokenName, const QString& backgroundToken,
                             double minimumContrast = 4.5) const;
    bool validateColorContrast(const QString& foregroundToken, const QString& backgroundToken,
...
private:
    ...
    QVariant resolveTokenWithFallback(const QString& tokenName, const QVariant& fallback) const;
...
namespace TokenUtils {
    ...
    inline QMargins margins(const QString& component, const QString& size = "medium") {
        return FluentDesignTokenUtils::instance().getComponentMargins(component, size);
    }
    ...
    inline QSize componentSize(const QString& component, const QString& size = "medium") {
...
#define FLUENT_COLOR(token) \
    FluentQt::Styling::TokenUtils::color(token)
...
#define FLUENT_FONT(token) \
    FluentQt::Styling::TokenUtils::font(token)

#define FLUENT_TITLE_FONT(level) \
    FluentQt::Styling::TokenUtils::titleFont(level)

#define FLUENT_BODY_FONT(strong) \
    FluentQt::Styling::TokenUtils::bodyFont(strong)

#define FLUENT_SPACING(size) \
    FluentQt::Styling::TokenUtils::spacing(size)

#define FLUENT_MARGINS(component, size) \
    FluentQt::Styling::TokenUtils::margins(component, size)

#define FLUENT_SIZE(token) \
    FluentQt::Styling::TokenUtils::size(token)

#define FLUENT_COMPONENT_SIZE(component, size) \
    FluentQt::Styling::TokenUtils::componentSize(component, size)

#define FLUENT_DURATION(speed) \
    FluentQt::Styling::TokenUtils::duration(speed)

#define FLUENT_EASING(type) \
    FluentQt::Styling::TokenUtils::easing(type)
...
#define FLUENT_VALIDATE_TOKEN(token) \
    do { \
        if (!FluentQt::Styling::FluentDesignTokenUtils::instance().isTokenValid(token)) { \
            qWarning() << "Invalid design token:" << token; \
        } \
    } while(0)
#else
#define FLUENT_VALIDATE_TOKEN(token) do {} while(0)
#endif
