...
private slots:
        ...
        // Required field validation
            ...
            if (value.isEmpty()) {
                setFieldError(textInput,
                              QString("%1 is required").arg(fieldName));
            } else {
                setFieldSuccess(textInput,
                                QString("Valid %1").arg(fieldName.toLower()));
            }
        }
    ...
    void setFieldError(FluentTextInput* field, const QString& message) {
        ...
        QString announcement =
            QString("Error in %1: %2").arg(field->accessibleName(), message);
    ...
    void updateFormValidation() {
        ...
        for (auto* input :
             {m_firstNameInput, m_lastNameInput, m_emailInput, m_phoneInput}) {
        ...
        if (isValid) {
            ...
            m_formStatusLabel->setText(
                QString("%1 error(s) found").arg(errors.size()));
    ...
    void submitForm() {
        if (!m_submitButton->isEnabled()) {
            announceToScreenReader(
                "Cannot submit form: Please fix validation errors first");
            return;
        }
        ...
        QTimer::singleShot(2000, [this]() {
            ...
            FluentToastGlobal::showSuccess("Success",
                                           "Form submitted successfully!");
            announceToScreenReader("Form submitted successfully");

            // Reset form
            QTimer::singleShot(1000, [this]() { resetForm(); });
        });
    }
    ...
    void toggleHighContrast() {
        ...
        m_contrastButton->setText(isHighContrast ? "Enable High Contrast"
...
private:
    ...
    void setupAccessibilityToolbar(QVBoxLayout* layout) {
        ...
        auto* shortcutsLabel = new QLabel(
            "Shortcuts: Alt+1 (Personal), Alt+2 (Contact), Alt+3 "
            "(Preferences), Ctrl+Alt+S (Submit)");
        ...
        m_contrastButton->setAccessibleDescription(
            "Switch between normal and high contrast display modes");
        connect(m_contrastButton, &FluentButton::clicked, this,
                &AccessibleFormBuilder::toggleHighContrast);
        ...
        helpButton->setAccessibleDescription(
            "Display keyboard shortcuts and accessibility features");
    ...
    void setupContactInfoSection(QVBoxLayout* layout) {
        ...
        m_emailInput->setAccessibleDescription(
        ...
        m_phoneInput->setAccessibleDescription(
        ...
        m_countryCombo->setAccessibleDescription(
    ...
    void setupPreferencesSection(QVBoxLayout* layout) {
        ...
        m_marketingCheck->setAccessibleDescription(
    ...
    void setupFormActions(QVBoxLayout* layout) {
        ...
        resetButton->setAccessibleDescription(
            "Clear all form fields and start over");
        connect(resetButton, &FluentButton::clicked, this,
                &AccessibleFormBuilder::resetForm);
    ...
    void setupStatusBar(QVBoxLayout* layout) {
        ...
        m_formStatusLabel =
            new QLabel("Fill out required fields to enable submission");
    ...
    void setupAccessibility() {
        ...
        setAccessibleDescription(
            "A demonstration of accessible form design with Element Fluent UI "
            "components");
        ...
        for (auto* input :
            ...
            connect(input, &FluentTextInput::textChanged, [this, input]() {
                ...
                QTimer::singleShot(500,
                                   [this, input]() { validateField(input); });
            });
        }

        connect(m_submitButton, &FluentButton::clicked, this,
                &AccessibleFormBuilder::submitForm);
