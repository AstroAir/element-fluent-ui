...
private:
    ...
    void measureAnimationPerformance(const QList<QAbstractAnimation*>& animations);
...
void FluentAnimationPerformanceTest::testConcurrentAnimationPerformance() {
    ...
    m_animationManager->setOptimizationStrategy(FluentOptimizationStrategy::BatchAnimations, false);
    ...
    m_animationManager->setOptimizationStrategy(FluentOptimizationStrategy::BatchAnimations, true);
    ...
    qDebug() << "  Improvement:" << QString::number((double)unbatchedTime / batchedTime, 'f', 2) << "x";
...
void FluentAnimationPerformanceTest::testAnimationBatchingEfficiency() {
    ...
    for (auto* button : buttons) {
        ...
        animations.append(FluentAnimator::slideUp(button, QPoint(0, 20)).release());
...
void FluentAnimationPerformanceTest::testGPUAccelerationPerformance() {
    ...
    if (!FluentGPURenderer::isCapabilitySupported(FluentGPUCapability::OpenGL_3_3)) {
    ...
    if (gpuTime < cpuTime) {
        ...
        qDebug() << "  GPU improvement:" << QString::number(improvement, 'f', 2) << "x";
...
void FluentAnimationPerformanceTest::testAnimationMemoryUsage() {
    ...
    for (auto* button : buttons) {
        animations.append(FluentAnimator::fadeIn(button).release());
        animations.append(FluentAnimator::scaleIn(button).release());
        animations.append(FluentAnimator::slideUp(button, QPoint(0, 10)).release());
...
void FluentAnimationPerformanceTest::testFrameRateOptimization() {
    ...
    m_animationManager->setOptimizationStrategy(FluentOptimizationStrategy::AdaptiveFrameRate, true);
...
void FluentAnimationPerformanceTest::createTestButtons(int count, QList<FluentButton*>& buttons) {
