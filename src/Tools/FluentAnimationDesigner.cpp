...
FluentAnimationPreset FluentAnimationPreset::createCustomPreset(
...
FluentAnimationDesigner::FluentAnimationDesigner(QWidget* parent)
    ...
    connect(m_previewTimer, &QTimer::timeout, this,
            &FluentAnimationDesigner::onPreviewUpdate);
...
void FluentAnimationDesigner::setupMenuBar() {
    ...
    fileMenu->addAction("&New Project", this,
                        &FluentAnimationDesigner::newProject,
                        QKeySequence::New);
    fileMenu->addAction(
        "&Open Project...",
        [this]() {
            QString fileName = QFileDialog::getOpenFileName(
                this, "Open Project", "", "Animation Projects (*.fap)");
            if (!fileName.isEmpty()) {
    ...
    fileMenu->addAction(
        ...
        [this]() {
            QString fileName = QFileDialog::getSaveFileName(
                this, "Save Project", "", "Animation Projects (*.fap)");
            if (!fileName.isEmpty()) {
    ...
    fileMenu->addAction("&Export Animation...", [this]() {
        QString fileName = QFileDialog::getSaveFileName(
            this, "Export Animation", "", "C++ Files (*.cpp)");
        if (!fileName.isEmpty()) {
            exportAnimation("cpp");
        }
    });
    ...
    editMenu->addAction("&Add Layer",
                        [this]() { addAnimationLayer("New Layer"); });
    ...
    animationMenu->addAction("&Play", this,
                             &FluentAnimationDesigner::playAnimation,
                             QKeySequence("Space"));
    animationMenu->addAction("&Pause", this,
                             &FluentAnimationDesigner::pauseAnimation);
    animationMenu->addAction("&Stop", this,
                             &FluentAnimationDesigner::stopAnimation,
                             QKeySequence("Escape"));
    ...
    helpMenu->addAction("&About", [this]() {
        QMessageBox::about(
            this, "About Fluent Animation Designer",
...
void animateWidget(QWidget* target) {
    ...
    if (!m_timelineItems.isEmpty()) {
        ...
        animationType =
            item.property.contains("opacity") ? "fadeIn" : "slideUp";
    }

    QString finalCode =
        cppCode.arg(duration).arg(easingName).arg(animationType);
