...
struct FluentToastManagerConfig {
    ...
    QWidget* parentWidget{
        nullptr};  // Parent widget for positioning (null = screen)
    QEasingCurve easingCurve{QEasingCurve::OutCubic};
};

class FluentToastManager : public QObject {
    Q_OBJECT
    Q_PROPERTY(FluentToastPosition defaultPosition READ defaultPosition WRITE
                   setDefaultPosition NOTIFY defaultPositionChanged)
    Q_PROPERTY(int maxVisible READ maxVisible WRITE setMaxVisible NOTIFY
                   maxVisibleChanged)
    Q_PROPERTY(
        int maxQueued READ maxQueued WRITE setMaxQueued NOTIFY maxQueuedChanged)
    Q_PROPERTY(int stackSpacing READ stackSpacing WRITE setStackSpacing NOTIFY
                   stackSpacingChanged)
    Q_PROPERTY(int screenMargin READ screenMargin WRITE setScreenMargin NOTIFY
                   screenMarginChanged)
    Q_PROPERTY(bool allowDuplicates READ allowDuplicates WRITE
                   setAllowDuplicates NOTIFY allowDuplicatesChanged)
    Q_PROPERTY(bool stackToasts READ stackToasts WRITE setStackToasts NOTIFY
                   stackToastsChanged)

public:
    explicit FluentToastManager(QObject* parent = nullptr);
    explicit FluentToastManager(const FluentToastManagerConfig& config,
    ...
    FluentToastPosition defaultPosition() const {
        return m_config.defaultPosition;
    }
    ...
    // Toast management
    FluentToast* show(FluentToastType type, const QString& title,
                      const QString& message = QString());
    FluentToast* show(FluentToastType type, const QString& title,
                      const QString& message, FluentToastPosition position);
    FluentToast* show(FluentToastType type, const QString& title,
                      const QString& message, const FluentToastConfig& config);
    FluentToast* show(FluentToast* toast);
    FluentToast* show(FluentToast* toast, FluentToastPosition position);

    // Convenience methods
    FluentToast* showInfo(const QString& title,
                          const QString& message = QString());
    FluentToast* showSuccess(const QString& title,
                             const QString& message = QString());
    FluentToast* showWarning(const QString& title,
                             const QString& message = QString());
    FluentToast* showError(const QString& title,
                           const QString& message = QString());
    FluentToast* showCustom(const QIcon& icon, const QString& title,
                            const QString& message = QString());

    // Position-specific convenience methods
    FluentToast* showInfoAt(FluentToastPosition position, const QString& title,
                            const QString& message = QString());
    FluentToast* showSuccessAt(FluentToastPosition position,
                               const QString& title,
                               const QString& message = QString());
    FluentToast* showWarningAt(FluentToastPosition position,
                               const QString& title,
                               const QString& message = QString());
    FluentToast* showErrorAt(FluentToastPosition position, const QString& title,
                             const QString& message = QString());
    ...
    QRect getToastRect(FluentToastPosition position, const QSize& toastSize,
                       int index = 0) const;
    QPoint getToastPosition(FluentToastPosition position,
                            const QSize& toastSize, int index = 0) const;
...
private:
    ...
    QRect calculateToastRect(FluentToastPosition position,
                             const QSize& toastSize, int stackIndex) const;
    QPoint calculateToastPosition(FluentToastPosition position,
                                  const QSize& toastSize, int stackIndex) const;
