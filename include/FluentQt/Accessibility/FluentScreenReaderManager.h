...
public:
    ...
    FluentScreenReaderType getActiveScreenReader() const {
        return m_activeScreenReader;
    }
    ...
    void setAriaProperties(QWidget* widget,
                           const FluentAriaProperties& properties);
    ...
    void setAriaValue(QWidget* widget, double value, double min = 0.0,
                      double max = 100.0);
    ...
    // Live regions
    void setLiveRegion(QWidget* widget, FluentLiveRegionType type,
                       bool atomic = false);
    ...
    // Landmark management
    void setLandmark(QWidget* widget, FluentAriaRole landmarkRole,
                     const QString& label = "");
    ...
    void announceContentChanged(QWidget* widget, const QString& oldContent,
                                const QString& newContent);
    void announceStateChange(QWidget* widget, const QString& property,
                             const QString& newValue);

    // Screen reader specific optimizations
    void optimizeForScreenReader(FluentScreenReaderType type);
    void setScreenReaderSpecificProperty(QWidget* widget,
                                         const QString& property,
                                         const QString& value);
    QString getScreenReaderSpecificText(const QString& text,
                                        FluentScreenReaderType type);
    ...
    void setMaxAnnouncementLength(int characters) {
        m_maxAnnouncementLength = characters;
    }
    void setAnnouncementRate(int wordsPerMinute) {
        m_announcementRate = wordsPerMinute;
    }
    void enableVerboseMode(bool enabled) { m_verboseMode = enabled; }
    void enableContextualAnnouncements(bool enabled) {
        m_contextualAnnouncements = enabled;
    }
...
signals:
    ...
    void ariaPropertyChanged(QWidget* widget, const QString& property,
                             const QString& value);
    void liveRegionUpdated(QWidget* widget, const QString& content);
    void landmarkAdded(QWidget* widget, FluentAriaRole role,
                       const QString& label);
...
private:
    ...
    QString formatAnnouncementForScreenReader(const QString& message,
                                              FluentScreenReaderType type);
...
private:
    FluentScreenReaderType m_activeScreenReader{
        FluentScreenReaderType::Unknown};
