...
void FluentComponent::setSmoothTransitions(bool enabled) {
    if (m_smoothTransitions != enabled) {
        m_smoothTransitions = enabled;

        // Update animation properties based on ElaWidgetTools patterns
        if (m_stateAnimation) {
            if (enabled) {
                m_stateAnimation->setEasingCurve(QEasingCurve::OutCubic);
                m_stateAnimation->setDuration(m_transitionDuration);
            } else {
                m_stateAnimation->setDuration(0);
            }
        }

        emit smoothTransitionsChanged(enabled);
    }
}

void FluentComponent::setTransitionDuration(int duration) {
    if (m_transitionDuration != duration) {
        m_transitionDuration = duration;

        // Update existing animation duration
        if (m_stateAnimation && m_smoothTransitions) {
            m_stateAnimation->setDuration(duration);
        }

        emit transitionDurationChanged(duration);
    }
}
...
void FluentComponent::performStateTransition(FluentState from, FluentState to) {
    ...
    if (m_stateAnimation &&
        m_stateAnimation->state() == QAbstractAnimation::Running) {
