...
private:
    ...
    void createColorSwatches() {
        ...
        QList<QPair<QString, QColor*>> colorDefs = {
            {"Accent", nullptr},        {"Accent L1", nullptr},
            {"Accent L2", nullptr},     {"Success", nullptr},
            {"Warning", nullptr},       {"Error", nullptr},
            {"Neutral Light", nullptr}, {"Neutral", nullptr},
            {"Neutral Dark", nullptr}};

        int row = 0, col = 0;
        for (const auto& colorDef : colorDefs) {
            auto* colorSwatch =
                createColorSwatch(colorDef.first, QColor(128, 128, 128));
    ...
    QWidget* createColorSwatch(const QString& name, const QColor& color) {
        ...
        swatch->setStyleSheet(QString("QWidget { "
                                      "  background-color: %1; "
        ...
        nameLabel->setStyleSheet(
            "color: white; font-size: 10px; font-weight: bold; background: "
            "rgba(0,0,0,0.5); padding: 2px; border-radius: 2px;");
        ...
        colorLabel->setStyleSheet(
            "color: white; font-size: 9px; background: rgba(0,0,0,0.5); "
...
private slots:
    void onBaseColorChanged() {
        QColor color = QColorDialog::getColor(m_currentBaseColor, this,
                                              "Select Base Color");
        if (color.isValid()) {
            m_currentBaseColor = color;
            m_baseColorButton->setStyleSheet(
    ...
    void onAlgorithmChanged() { generatePalette(); }

    void onHarmonyChanged() { generatePalette(); }

    void onContrastChanged() {
        m_contrastLabel->setText(
            QString("Contrast: %1:1")
                .arg(m_contrastSlider->value() / 10.0, 0, 'f', 1));
        generatePalette();
    }

    void onAccessibilityToggled() { generatePalette(); }
...
private:
    void setupUI() {
        ...
        m_algorithmCombo->addItems({"Fluent Design", "Material Design",
                                    "Accessible", "Monochromatic", "Analogous",
                                    "Complementary", "Triadic"});

        m_harmonyCombo = new QComboBox(this);
        m_harmonyCombo->addItems({"Analogous", "Monochromatic", "Complementary",
                                  "Triadic", "Tetradic",
                                  "Split Complementary"});
        ...
        m_accessibilityCheck =
            new QCheckBox("Generate Accessible Variants", this);
        ...
        m_themeModeCombo->addItems(
            {"System", "Light", "Dark", "High Contrast"});

        m_variantCombo = new QComboBox(this);
        m_variantCombo->addItems(
            {"Default", "Compact", "Touch", "Dense", "Accessible"});
    ...
    void connectSignals() {
        connect(m_baseColorButton, &QPushButton::clicked, this,
                &ThemeControlPanel::onBaseColorChanged);
        connect(m_algorithmCombo,
                QOverload<int>::of(&QComboBox::currentIndexChanged), this,
                &ThemeControlPanel::onAlgorithmChanged);
        connect(m_harmonyCombo,
                QOverload<int>::of(&QComboBox::currentIndexChanged), this,
                &ThemeControlPanel::onHarmonyChanged);
        connect(m_contrastSlider, &QSlider::valueChanged, this,
                &ThemeControlPanel::onContrastChanged);
        connect(m_accessibilityCheck, &QCheckBox::toggled, this,
                &ThemeControlPanel::onAccessibilityToggled);
        connect(m_themeModeCombo,
                QOverload<int>::of(&QComboBox::currentIndexChanged), this,
                &ThemeControlPanel::onThemeModeChanged);
        connect(m_variantCombo,
                QOverload<int>::of(&QComboBox::currentIndexChanged), this,
                &ThemeControlPanel::onVariantChanged);
        connect(m_dynamicThemingCheck, &QCheckBox::toggled, this,
                &ThemeControlPanel::onDynamicThemingToggled);
        connect(m_generateButton, &QPushButton::clicked, this,
                &ThemeControlPanel::generatePalette);
    }
    ...
    void generatePalette() {
        ...
        if (algorithm == "Material Design") {
            config.algorithm =
                Styling::FluentThemeGenerationAlgorithm::MaterialDesign;
        } else if (algorithm == "Accessible") {
            config.algorithm =
                Styling::FluentThemeGenerationAlgorithm::Accessible;
        } else if (algorithm == "Monochromatic") {
            config.algorithm =
                Styling::FluentThemeGenerationAlgorithm::Monochromatic;
        } else if (algorithm == "Analogous") {
            config.algorithm =
                Styling::FluentThemeGenerationAlgorithm::Analogous;
        } else if (algorithm == "Complementary") {
            config.algorithm =
                Styling::FluentThemeGenerationAlgorithm::Complementary;
        } else if (algorithm == "Triadic") {
            config.algorithm = Styling::FluentThemeGenerationAlgorithm::Triadic;
        } else {
            config.algorithm =
                Styling::FluentThemeGenerationAlgorithm::FluentDesign;
        }
        ...
        // Generate palette
        Styling::FluentColorPalette palette =
            themeManager.generateColorPalette(config);
...
public:
    explicit AdvancedThemingMainWindow(QWidget* parent = nullptr)
        : QMainWindow(parent) {
        setupUI();
        connectSignals();
...
private slots:
    void onThemeChanged() { updateThemeDisplay(); }
...
private:
    void setupUI() {
        ...
        m_lightPaletteWidget =
            new ColorPaletteWidget("Light Theme Palette", this);
        m_darkPaletteWidget =
            new ColorPaletteWidget("Dark Theme Palette", this);
    ...
    void createComponentPreview(QVBoxLayout* layout) {
        ...
        auto* sampleText =
            new QLabel("Sample text with current theme colors", this);
    ...
    void connectSignals() {
        connect(m_controlPanel, &ThemeControlPanel::themeChanged, this,
                &AdvancedThemingMainWindow::onThemeChanged);
        connect(m_controlPanel, &ThemeControlPanel::paletteGenerated, this,
                &AdvancedThemingMainWindow::onPaletteGenerated);

        // Connect to theme manager signals
        auto& themeManager = Styling::FluentAdvancedThemeManager::instance();
        connect(&themeManager,
                &Styling::FluentAdvancedThemeManager::colorPaletteGenerated,
