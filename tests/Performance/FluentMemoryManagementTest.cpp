...
void FluentMemoryManagementTest::testMemoryPoolPerformance() {
    ...
    for (int i = 0; i < allocationCount; ++i) {
        auto* button =
            buttonPool.construct(QString("Button %1").arg(i), m_testWidget);
    ...
    qDebug() << "Memory pool performance (" << allocationCount
             << "allocations):";
    ...
    if (pooledAllocationTime < standardAllocationTime) {
        double improvement =
            (double)standardAllocationTime / pooledAllocationTime;
        qDebug() << "  Pool improvement:"
                 << QString::number(improvement, 'f', 2) << "x";
...
void FluentMemoryManagementTest::testAdvancedMemoryManagerEfficiency() {
    ...
    for (int i = 0; i < componentCount; ++i) {
        ...
        if (button) {
            new (button)
                FluentButton(QString("Button %1").arg(i), m_testWidget);
    ...
    qDebug() << "  Memory after allocation:" << memoryAfterAllocation
             << "bytes";
...
void FluentMemoryManagementTest::testComponentCaching() {
    ...
    for (int i = 0; i < testCount; ++i) {
        auto* button =
            m_memoryManager->allocateComponent<FluentButton>("FluentButton");
        if (button) {
            new (button)
                FluentButton(QString("Button %1").arg(i), m_testWidget);
            buttons.append(button);
...
void FluentMemoryManagementTest::testMemoryStrategies() {
    const QList<FluentMemoryStrategy> strategies = {
        FluentMemoryStrategy::Standard, FluentMemoryStrategy::Pooled,
        FluentMemoryStrategy::Cached, FluentMemoryStrategy::Adaptive};
    ...
    for (FluentMemoryStrategy strategy : strategies) {
        ...
        QString strategyName =
            QString("TestStrategy_%1").arg(static_cast<int>(strategy));
        ...
        for (int i = 0; i < allocationCount; ++i) {
            auto* button =
                m_memoryManager->allocateComponent<FluentButton>(strategyName);
            if (button) {
                new (button)
                    FluentButton(QString("Button %1").arg(i), m_testWidget);
                buttons.append(button);
...
void FluentMemoryManagementTest::testMemoryLimitEnforcement() {
    ...
    qDebug() << "  Allocated" << buttons.size()
             << "components before limit exceeded";
