...
private:
    ...
    void updateLocaleInfo() {
        ...
        m_nativeNameLabel->setText(m_currentLocale.nativeLanguageName() + " (" + m_currentLocale.nativeCountryName() + ")");
        m_languageLabel->setText(QLocale::languageToString(m_currentLocale.language()));
        m_countryLabel->setText(QLocale::countryToString(m_currentLocale.country()));
        m_scriptLabel->setText(QLocale::scriptToString(m_currentLocale.script()));
        
        // Text direction
        bool isRTL = rtlManager.isRTLLocale(m_currentLocale);
        m_textDirectionLabel->setText(isRTL ? "Right-to-Left (RTL)" : "Left-to-Right (LTR)");
        ...
        // Cultural context
        auto culturalConfig = culturalManager.getCulturalConfig(m_currentLocale);
        culturalInfo += QString("Cultural Context: %1\n").arg(
            culturalConfig.context == Core::FluentCulturalContext::HighContext ? "High Context" : "Low Context"
        );
        
        // Layout preferences
        culturalInfo += QString("Layout Style: %1\n").arg(
            culturalConfig.layoutStyle == Core::FluentCulturalLayoutStyle::Minimal ? "Minimal" : "Rich"
        );
        
        // Interaction style
        culturalInfo += QString("Interaction Style: %1\n").arg(
            culturalConfig.interactionStyle == Core::FluentCulturalInteraction::Formal ? "Formal" : "Casual"
        );
...
private:
    ...
    void updateFormattingExamples() {
        ...
        auto decimalResult = formatter.formatNumber(123456.789, m_currentLocale);
        m_decimalLabel->setText(decimalResult.formattedText);
        
        auto currencyResult = formatter.formatCurrency(1234.56, "USD", m_currentLocale);
        m_currencyLabel->setText(currencyResult.formattedText);
        
        auto percentResult = formatter.formatPercentage(0.1234, m_currentLocale);
        m_percentageLabel->setText(percentResult.formattedText);
        
        auto compactResult = formatter.formatCompactNumber(1234567, m_currentLocale);
        ...
        auto relativeResult = formatter.formatRelativeDateTime(now.addSecs(-3600), m_currentLocale);
        m_relativeLabel->setText(relativeResult.formattedText);
        
        auto durationResult = formatter.formatDuration(3661000, m_currentLocale); // 1 hour, 1 minute, 1 second
        ...
        auto nameResult = formatter.formatName("John", "Smith", m_currentLocale);
        ...
        auto addressResult = formatter.formatAddress(addressComponents, m_currentLocale);
        m_addressLabel->setText(addressResult.formattedText);
        
        auto phoneResult = formatter.formatPhoneNumber("+1-555-123-4567", m_currentLocale);
...
private:
    void setupUI() {
        ...
        m_mixedText = new QLabel("English text with عربي and עברית mixed in", this);
        ...
        connect(m_enableRTLCheck, &QCheckBox::toggled, this, &RTLDemoWidget::onRTLToggled);
        connect(m_mirrorLayoutCheck, &QCheckBox::toggled, this, &RTLDemoWidget::onMirrorToggled);
        connect(m_adaptTextCheck, &QCheckBox::toggled, this, &RTLDemoWidget::onTextAdaptToggled);
...
private slots:
    ...
    void onMirrorToggled(bool enabled) {
        if (enabled) {
            ...
            auto* layout = qobject_cast<QHBoxLayout*>(m_button1->parentWidget()->layout());
            if (layout) {
    ...
    void onTextAdaptToggled(bool enabled) {
        if (enabled) {
            ...
            auto arabicDir = rtlManager.detectTextDirection(m_arabicText->text());
            auto hebrewDir = rtlManager.detectTextDirection(m_hebrewText->text());
            auto mixedDir = rtlManager.detectTextDirection(m_mixedText->text());
            
            m_arabicText->setAlignment(arabicDir == Core::FluentTextDirection::RightToLeft ? Qt::AlignRight : Qt::AlignLeft);
            m_hebrewText->setAlignment(hebrewDir == Core::FluentTextDirection::RightToLeft ? Qt::AlignRight : Qt::AlignLeft);
            m_mixedText->setAlignment(mixedDir == Core::FluentTextDirection::RightToLeft ? Qt::AlignRight : Qt::AlignLeft);
        }
    }
...
public:
    explicit InternationalizationMainWindow(QWidget* parent = nullptr) : QMainWindow(parent) {
        setupUI();
        connectSignals();
...
private:
    void setupUI() {
        ...
        m_culturalAdaptationCheck = new QCheckBox("Enable Cultural Adaptation", this);
    ...
    void connectSignals() {
        connect(m_localeCombo, QOverload<int>::of(&QComboBox::currentIndexChanged),
                this, &InternationalizationMainWindow::onLocaleChanged);
        connect(m_culturalAdaptationCheck, &QCheckBox::toggled,
                this, &InternationalizationMainWindow::onCulturalAdaptationToggled);
    }
    ...
    void setCurrentLocale(const QLocale& locale) {
        ...
        // Update status bar
        statusBar()->showMessage(QString("Current locale: %1").arg(locale.name()));
        
        // Update window title with locale
        setWindowTitle(QString("FluentQt Internationalization Demo - %1").arg(locale.nativeLanguageName()));
    }
