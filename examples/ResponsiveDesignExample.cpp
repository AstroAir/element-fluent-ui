...
public:
    explicit ResponsiveCard(const QString& title, const QString& content,
...
private:
    ...
    void updateLayoutForBreakpoint(Core::FluentBreakpoint breakpoint) {
        ...
        auto& typographyManager =
            Core::FluentFluidTypographyManager::instance();
        QSize viewportSize =
            parentWidget() ? parentWidget()->size() : QSize(1024, 768);

        // Update title font
        QFont titleFont = typographyManager.calculateFluidFont(
            Core::FluentTypographyStyle::Title, viewportSize);
        m_titleLabel->setFont(titleFont);

        // Update content font
        QFont contentFont = typographyManager.calculateFluidFont(
            Core::FluentTypographyStyle::Body, viewportSize);
...
private slots:
    ...
    void onVariantChanged() {
        auto variant = static_cast<Core::FluentComponentVariant>(
            m_variantCombo->currentData().toInt());
...
private:
    void setupUI() {
        ...
        m_variantCombo->addItem(
            "Compact", static_cast<int>(Core::FluentComponentVariant::Compact));
        m_variantCombo->addItem(
            "Comfortable",
            static_cast<int>(Core::FluentComponentVariant::Comfortable));
        m_variantCombo->addItem(
            "Spacious",
            static_cast<int>(Core::FluentComponentVariant::Spacious));
        m_variantCombo->addItem(
            "Touch", static_cast<int>(Core::FluentComponentVariant::Touch));
        m_variantCombo->addItem(
            "Dense", static_cast<int>(Core::FluentComponentVariant::Dense));
        m_variantCombo->addItem(
            "Accessible",
            static_cast<int>(Core::FluentComponentVariant::Accessible));
        m_variantCombo->setCurrentIndex(1);  // Comfortable

        connect(m_variantCombo,
                QOverload<int>::of(&QComboBox::currentIndexChanged), this,
                &ResponsiveGridWidget::onVariantChanged);
    ...
    void setupResponsive() {
        ...
        connect(&responsiveManager,
                &Core::FluentResponsiveManager::breakpointChanged, this,
                &ResponsiveGridWidget::onBreakpointChanged);
    ...
    void createSampleCards() {
        QStringList titles = {
            ...
            "Component Variants",     "Layout Systems",
            "Animation Framework",    "Theming System",
            "Internationalization"};

        QStringList contents = {
            "Real-time performance tracking with automated optimization "
            "suggestions.",
            "WCAG 2.1 AA compliance with screen reader support and keyboard "
            "navigation.",
            ...
            "Full internationalization with RTL support and locale-aware "
            "formatting."};
        ...
        for (int i = 0; i < titles.size(); ++i) {
            ...
            Core::FluentResponsiveComponentManager::instance()
...
public:
    explicit ResponsiveMainWindow(QWidget* parent = nullptr)
        : QMainWindow(parent) {
        setupUI();
        setupResponsive();
        startResizeTimer();
...
private slots:
    void onResizeTimer() {
        ...
        QFont titleFont = typographyManager.calculateFluidFont(
            Core::FluentTypographyStyle::Headline, size());
...
private:
    void setupUI() {
        ...
        connect(accessibleAction, &QAction::triggered, this, [this]() {
            setVariantForAllComponents(
                Core::FluentComponentVariant::Accessible);
        });
        ...
        connect(&responsiveManager,
                &Core::FluentResponsiveManager::breakpointChanged, this,
                [statusLabel](Core::FluentBreakpoint breakpoint) {
                    ...
                    switch (breakpoint) {
                        ...
                        case Core::FluentBreakpoint::XLarge:
                            ...
                            break;
                        case Core::FluentBreakpoint::XXLarge:
                            breakpointName = "XXLarge";
                            break;
                        default:
                            break;
                    }
                    statusLabel->setText(
                        QString("Breakpoint: %1").arg(breakpointName));
                });
    }
    ...
    void startResizeTimer() {
        ...
        connect(m_resizeTimer, &QTimer::timeout, this,
                &ResponsiveMainWindow::onResizeTimer);
    }
