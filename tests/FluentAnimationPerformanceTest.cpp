...
private slots:
    ...
    void cleanupTestCase() { delete m_testWidget; }
    ...
    void testAnimationPerformance() {
        ...
        for (int i = 0; i < buttonCount; ++i) {
            auto* button = new Components::FluentButton(
                QString("Button %1").arg(i), m_testWidget);
        ...
        QTimer::singleShot(500, [&]() {
            ...
            QVERIFY(
                elapsed <
                600);  // Should not take much longer than animation duration
    ...
    void testReducedMotionCompliance() {
        ...
        bool originalReducedMotion =
            Animation::FluentAnimator::shouldRespectReducedMotion();

        // Test with reduced motion enabled
        if (originalReducedMotion) {
            auto animation = Animation::FluentAnimator::fadeIn(button, config);

            // Animation should have zero duration when reduced motion is
            // enabled
    ...
    void testRippleEffect() {
        ...
        QSignalSpy finishedSpy(rippleAnimation.get(),
    ...
    void benchmarkAnimationThroughput() {
        ...
        qDebug() << "  Avg per animation:" << (double)totalTime / maxAnimations
                 << "ms";
    }
