...
private:
    ...
    bool comparePixmaps(const QPixmap& actual, const QPixmap& expected,
...
void FluentVisualRegressionTest::cleanupTestCase() { delete m_testWidget; }
...
void FluentVisualRegressionTest::testFluentButtonVisual() {
    ...
    QVERIFY2(comparePixmaps(actual, expected),
             "Button visual regression detected");
}
...
void FluentVisualRegressionTest::testFluentButtonStates() {
    ...
    // Test hover state
    QHoverEvent hoverEnter(QEvent::HoverEnter, QPointF(60, 60),
                           QPointF(60, 60));
    ...
    // Test pressed state
    QMouseEvent pressEvent(QEvent::MouseButtonPress, QPointF(60, 60),
    ...
    QVERIFY2(!comparePixmaps(normalState, hoverState, 0.99),
             "Button hover state should be visually different");
    QVERIFY2(!comparePixmaps(hoverState, pressedState, 0.99),
             "Button pressed state should be visually different");
    ...
    QVERIFY2(comparePixmaps(normalState, expectedNormal),
             "Button normal state visual regression");
    QVERIFY2(comparePixmaps(hoverState, expectedHover),
             "Button hover state visual regression");
    QVERIFY2(comparePixmaps(pressedState, expectedPressed),
             "Button pressed state visual regression");
}
...
void FluentVisualRegressionTest::testFluentTextInputVisual() {
    ...
    QVERIFY2(comparePixmaps(emptyState, expectedEmpty),
             "TextInput empty state visual regression");
    QVERIFY2(comparePixmaps(withTextState, expectedText),
             "TextInput with text visual regression");
    QVERIFY2(comparePixmaps(focusedState, expectedFocused),
             "TextInput focused state visual regression");
}
...
void FluentVisualRegressionTest::testFluentCardVisual() {
    ...
    card->setContent(
        "This is a test card with some content to verify visual appearance.");
    ...
    QVERIFY2(comparePixmaps(actual, expected),
             "Card visual regression detected");
}
...
void FluentVisualRegressionTest::testComplexLayoutVisual() {
    ...
    QVERIFY2(comparePixmaps(actual, expected),
             "Complex layout visual regression detected");
}

QPixmap FluentVisualRegressionTest::captureWidget(QWidget* widget,
...
bool FluentVisualRegressionTest::comparePixmaps(const QPixmap& actual,
...
void FluentVisualRegressionTest::saveReferenceImage(const QPixmap& pixmap,
                                                    const QString& testName) {
    QString filename =
        QString("%1/%2_reference.png").arg(m_testDataPath, testName);
    pixmap.save(filename, "PNG");
}

QPixmap FluentVisualRegressionTest::loadReferenceImage(
    const QString& testName) {
    QString filename =
        QString("%1/%2_reference.png").arg(m_testDataPath, testName);
