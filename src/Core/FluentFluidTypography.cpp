...
FluentFluidTypographyManager::FluentFluidTypographyManager() {
    ...
    connect(m_cacheCleanupTimer, &QTimer::timeout, this,
            &FluentFluidTypographyManager::cleanupCache);
    ...
    connect(qApp, &QApplication::fontDatabaseChanged, this,
            &FluentFluidTypographyManager::onSystemFontChanged);

    qDebug() << "FluentFluidTypographyManager initialized";
}

void FluentFluidTypographyManager::setConfig(
    const FluentFluidTypographyConfig& config) {
...
void FluentFluidTypographyManager::setTypographyScale(
    FluentTypographyScale scale) {
    QMutexLocker locker(&m_configMutex);
...
double FluentFluidTypographyManager::getScaleRatio(
    FluentTypographyScale scale) const {
    switch (scale) {
        case FluentTypographyScale::MinorSecond:
            return 1.067;
        case FluentTypographyScale::MajorSecond:
            return 1.125;
        case FluentTypographyScale::MinorThird:
            return 1.200;
        case FluentTypographyScale::MajorThird:
            return 1.250;
        case FluentTypographyScale::PerfectFourth:
            return 1.333;
        case FluentTypographyScale::AugmentedFourth:
            return 1.414;
        case FluentTypographyScale::PerfectFifth:
            return 1.500;
        case FluentTypographyScale::GoldenRatio:
            return 1.618;
        case FluentTypographyScale::Custom:
            return m_config.customScaleRatio;
    }
    return 1.250;  // Default to major third
}

void FluentFluidTypographyManager::registerStyle(
...
void FluentFluidTypographyManager::unregisterStyle(
...
FluentTypographyStyleDefinition
FluentFluidTypographyManager::getStyleDefinition(
    FluentTypographyStyle style) const {
    QMutexLocker locker(&m_configMutex);
    return m_styleDefinitions.value(style, FluentTypographyStyleDefinition{});
}

FluentCalculatedTypography
FluentFluidTypographyManager::calculateFluidTypography(
    ...
    if (m_config.enableResponsiveLineHeight) {
        result.lineHeight =
            calculateFluidLineHeight(result.fontSize, viewportSize);
    } else {
    ...
    if (m_config.enableResponsiveLetterSpacing) {
        result.letterSpacing =
            calculateFluidLetterSpacing(result.fontSize, viewportSize);
    } else {
    ...
    result.cssClass =
        QString("fluent-typography-%1").arg(static_cast<int>(style));
...
QFont FluentFluidTypographyManager::calculateFluidFont(
    FluentTypographyStyle style, const QSize& viewportSize) const {
...
QFont FluentFluidTypographyManager::calculateFluidFont(
    const QFont& baseFont, const QSize& viewportSize) const {
...
double FluentFluidTypographyManager::calculateFluidFontSize(
    double baseSize, const QSize& viewportSize) const {
    QMutexLocker locker(&m_configMutex);
    ...
    viewportWidth =
        clamp(viewportWidth, m_config.minViewport, m_config.maxViewport);
    ...
    double fluidSize = interpolate(minSize, maxSize, m_config.minViewport,
                                   m_config.maxViewport, viewportWidth);
...
double FluentFluidTypographyManager::calculateFluidLineHeight(
    double fontSize, const QSize& viewportSize) const {
...
double FluentFluidTypographyManager::calculateFluidLetterSpacing(
    double fontSize, const QSize& viewportSize) const {
    Q_UNUSED(viewportSize)
    ...
    return clamp(spacing, m_config.minLetterSpacing * fontSize,
                 m_config.maxLetterSpacing * fontSize);
}

void FluentFluidTypographyManager::setAccessibilityScaleFactor(double factor) {
    factor = clamp(factor, m_config.minAccessibilityScale,
                   m_config.maxAccessibilityScale);
...
double FluentFluidTypographyManager::interpolate(double min, double max,
                                                 double minViewport,
                                                 double maxViewport,
                                                 double currentViewport) const {
    if (maxViewport <= minViewport) {
        return min;
    }

    double ratio =
        (currentViewport - minViewport) / (maxViewport - minViewport);
    ratio = clamp(ratio, 0.0, 1.0);

    return min + (max - min) * ratio;
}

double FluentFluidTypographyManager::clamp(double value, double min,
                                           double max) const {
    return std::max(min, std::min(max, value));
}

double FluentFluidTypographyManager::applyAccessibilityScaling(
    double value) const {
...
QFont FluentFluidTypographyManager::createFontFromDefinition(
    const FluentTypographyStyleDefinition& definition, double fontSize) const {
    QFont font;
...
QString FluentFluidTypographyManager::generateCacheKey(
    FluentTypographyStyle style, const QSize& viewportSize) const {
...
void FluentFluidTypographyManager::addToCache(
    const QString& key, const FluentCalculatedTypography& typography) const {
    QMutexLocker locker(&m_cacheMutex);
...
bool FluentFluidTypographyManager::getFromCache(
    const QString& key, FluentCalculatedTypography& typography) const {
    QMutexLocker locker(&m_cacheMutex);
...
bool FluentFluidTypographyManager::validateConfigInternal(
    const FluentFluidTypographyConfig& config) const {
    return isValidSize(config.baseSize) && isValidSize(config.minSize) &&
           isValidSize(config.maxSize) && config.minSize < config.maxSize &&
...
QMap<QString, QString> FluentFluidTypographyManager::generateCSSProperties(
    ...
    if (std::abs(typography.letterSpacing) > 0.01) {
        properties["letter-spacing"] =
            QString("%1px").arg(typography.letterSpacing);
    }

    properties["font-weight"] =
        QString::number(static_cast<int>(definition.weight));
