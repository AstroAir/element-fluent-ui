...
private slots:
    void demonstrateMicroInteractions() {
        ...
        hoverConfig.duration =
            std::chrono::milliseconds(150);  // Fluent standard
        hoverConfig.easing = Animation::FluentEasing::EaseOutQuad;

        auto hoverAnim = Animation::FluentAnimator::hoverEffect(m_primaryButton,
                                                                hoverConfig);
        hoverAnim->start();
    ...
    void demonstratePerformantTransitions() {
        ...
        auto scaleAnim =
            Animation::FluentAnimator::scaleIn(m_demoCard, scaleConfig);
        scaleAnim->start();

        // Staggered entrance for multiple elements
        QTimer::singleShot(100, [this]() { demonstrateStaggeredAnimations(); });
    }
    ...
    void demonstrateStaggeredAnimations() {
        ...
        for (int i = 0; i < buttons.size(); ++i) {
            QTimer::singleShot(i * 100, [this, buttons, i]() {
                ...
                auto slideAnim = Animation::FluentAnimator::slideUp(
                    buttons[i], 30, slideConfig);
                slideAnim->start();
            });
        }
    }
    ...
    void demonstrateReducedMotion() {
        ...
        auto fadeAnim =
            Animation::FluentAnimator::fadeIn(m_statusLabel, config);
        fadeAnim->start();
    }
...
private:
    void setupUI() {
        ...
        m_demoCard->setDescription(
            "Demonstrating hardware-accelerated, Fluent Design compliant "
            "animations");
        ...
        connect(m_primaryButton, &Components::FluentButton::clicked, this,
                &OptimizedAnimationDemo::demonstrateMicroInteractions);
        buttonLayout->addWidget(m_primaryButton);

        auto* transitionButton =
            new Components::FluentButton("Performant Transitions");
        connect(transitionButton, &Components::FluentButton::clicked, this,
                &OptimizedAnimationDemo::demonstratePerformantTransitions);
        buttonLayout->addWidget(transitionButton);

        auto* accessibilityButton =
            new Components::FluentButton("Reduced Motion");
        connect(accessibilityButton, &Components::FluentButton::clicked, this,
                &OptimizedAnimationDemo::demonstrateReducedMotion);
    ...
    void setupOptimizedAnimations() {
        ...
        QTimer::singleShot(100, [this]() {
            ...
            auto slideAnim =
                Animation::FluentAnimator::slideUp(this, 50, entranceConfig);
            slideAnim->start();
        });
    }
