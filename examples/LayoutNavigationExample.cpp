...
private slots:
    ...
    void onPaddingChanged(int value) { m_demoPanel->setPadding(value); }

    void onCollapsibleToggled(bool checked) {
        m_demoPanel->setCollapsible(checked);
    }

    void onAnimatedToggled(bool checked) { m_demoPanel->setAnimated(checked); }
    ...
    void onBreadcrumbSeparatorChanged(int index) {
        auto separator =
            static_cast<Components::FluentBreadcrumbSeparator>(index);
    ...
    void onCreateNewPanel() {
        ...
        auto* newPanel = new Components::FluentPanel(
            QString("Dynamic Panel %1").arg(panelCounter++), this);
        newPanel->setCollapsible(true);

        auto* layout = newPanel->createVerticalLayout();
        layout->addWidget(
            new QLabel("This is a dynamically created panel", this));
...
private:
    void setupUI() {
        ...
        // Title
        auto* titleLabel =
            new QLabel("Fluent UI Layout & Navigation Components Demo", this);
        auto& theme = Styling::FluentTheme::instance();
    ...
    void setupBreadcrumbSection(QVBoxLayout* mainLayout) {
        ...
        m_breadcrumbStyleCombo->addItems(
            {"Default", "Compact", "Pills", "Underline"});
        ...
        m_breadcrumbSeparatorCombo->addItems(
            {"Chevron", "Slash", "Backslash", "Pipe", "Arrow", "Dot"});
        ...
        connect(m_breadcrumbStyleCombo,
                QOverload<int>::of(&QComboBox::currentIndexChanged), this,
                &LayoutNavigationWindow::onBreadcrumbStyleChanged);
        connect(m_breadcrumbSeparatorCombo,
                QOverload<int>::of(&QComboBox::currentIndexChanged), this,
                &LayoutNavigationWindow::onBreadcrumbSeparatorChanged);
        connect(m_breadcrumb, &Components::FluentBreadcrumb::itemClicked, this,
                &LayoutNavigationWindow::onBreadcrumbItemClicked);
        connect(addItemButton, &QPushButton::clicked, this,
                &LayoutNavigationWindow::onAddBreadcrumbItem);
        connect(removeItemButton, &QPushButton::clicked, this,
                &LayoutNavigationWindow::onRemoveBreadcrumbItem);
    }

    void setupControlsPanel(QSplitter* splitter) {
        auto* controlsPanel =
            new Components::FluentPanel("Panel Controls", this);
        ...
        m_panelTypeCombo->addItems({"Card", "Surface", "Acrylic", "Mica",
                                    "Sidebar", "Content", "Dialog", "Flyout"});
        ...
        // Connect controls
        connect(m_panelTypeCombo,
                QOverload<int>::of(&QComboBox::currentIndexChanged), this,
                &LayoutNavigationWindow::onPanelTypeChanged);
        connect(m_elevationCombo,
                QOverload<int>::of(&QComboBox::currentIndexChanged), this,
                &LayoutNavigationWindow::onElevationChanged);
        connect(m_cornerRadiusCombo,
                QOverload<int>::of(&QComboBox::currentIndexChanged), this,
                &LayoutNavigationWindow::onCornerRadiusChanged);
        connect(m_paddingSpinBox, QOverload<int>::of(&QSpinBox::valueChanged),
                this, &LayoutNavigationWindow::onPaddingChanged);
        connect(m_collapsibleCheckBox, &QCheckBox::toggled, this,
                &LayoutNavigationWindow::onCollapsibleToggled);
        connect(m_animatedCheckBox, &QCheckBox::toggled, this,
                &LayoutNavigationWindow::onAnimatedToggled);
        connect(createPanelButton, &QPushButton::clicked, this,
                &LayoutNavigationWindow::onCreateNewPanel);
        connect(themeButton, &QPushButton::clicked, this,
                &LayoutNavigationWindow::onThemeToggle);
    }
    ...
    void setupDemoArea(QSplitter* splitter) {
        ...
        panelLayout->addWidget(
            new QLabel("This is a demo panel that you can configure using the "
