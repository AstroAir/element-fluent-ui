...
enum class FluentChartTheme {
    Light,
    Dark,
    Auto
};

enum class FluentLegendPosition {
    Top,
    Bottom,
    Left,
    Right,
...
class FluentChartView : public Core::FluentComponent {
    ...
    Q_PROPERTY(FluentChartTheme theme READ theme WRITE setTheme NOTIFY themeChanged)
    Q_PROPERTY(FluentLegendPosition legendPosition READ legendPosition WRITE setLegendPosition)
...
public:
    ...
    void setXAxisType(const QString& type);  // "value", "category", "datetime", "log"
...
private:
    ...
    void paintToolTip(QPainter& painter, const QPointF& pos, const QString& text);
