...
private slots:
    ...
    void editCurrentCustomer() {
        if (m_currentCustomerId == -1) return;
    ...
    void saveCustomer() {
        ...
        QTimer::singleShot(1000, [this, customer]() {
            if (m_isEditing) {
                ...
                FluentToastGlobal::showSuccess("Updated", "Customer updated successfully!");
            } else {
                customer.id = m_nextCustomerId++;
                m_customers.append(customer);
                addCustomerToTree(customer);
                FluentToastGlobal::showSuccess("Added", "New customer added successfully!");
            }
            
            m_saveButton->setLoading(false);
            m_saveButton->setText(m_isEditing ? "Update Customer" : "Add Customer");
    ...
    void deleteCurrentCustomer() {
        ...
        if (reply == QMessageBox::Yes) {
            m_customers.removeIf([this](const Customer& c) { return c.id == m_currentCustomerId; });
            refreshCustomerTree();
            clearCustomerForm();
            m_currentCustomerId = -1;
            FluentToastGlobal::showInfo("Deleted", "Customer deleted successfully.");
        }
    }
    ...
    void exportCustomers() {
        QString fileName = QFileDialog::getSaveFileName(this, "Export Customers", 
                                                       "customers.json", "JSON Files (*.json)");
        ...
        QTimer::singleShot(1500, [this, fileName]() {
            ...
            if (file.open(QIODevice::WriteOnly)) {
                ...
                FluentToastGlobal::showSuccess("Exported", 
                    QString("Exported %1 customers to %2").arg(m_customers.size()).arg(fileName));
            } else {
                FluentToastGlobal::showError("Export Failed", "Could not write to file.");
            }
...
private:
    ...
    void setupToolbar(QVBoxLayout* layout) {
        ...
        connect(addButton, &FluentButton::clicked, this, &CustomerManagementSystem::addNewCustomer);
    ...
    void setupCustomerList(QSplitter* splitter) {
        ...
        m_customerModel->setHorizontalHeaderLabels({"Name", "Company", "Status", "Value"});
        ...
        m_customerTree->header()->setSectionResizeMode(1, QHeaderView::ResizeToContents);
        m_customerTree->header()->setSectionResizeMode(2, QHeaderView::ResizeToContents);
        m_customerTree->header()->setSectionResizeMode(3, QHeaderView::ResizeToContents);
    ...
    void setupCustomerDetails(QSplitter* splitter) {
        ...
        m_countryCombo->addItems({"USA", "Canada", "UK", "Germany", "France", "Australia", "Japan"});
        ...
        connect(editButton, &FluentButton::clicked, this, &CustomerManagementSystem::editCurrentCustomer);
        
        auto* deleteButton = new FluentButton("Delete");
        deleteButton->setButtonStyle(FluentButtonStyle::Subtle);
        connect(deleteButton, &FluentButton::clicked, this, &CustomerManagementSystem::deleteCurrentCustomer);
    ...
    void loadSampleData() {
        ...
        QList<Customer> sampleCustomers = {
            {1, "John Smith", "john.smith@email.com", "+1-555-0123", "Tech Corp", "123 Main St", "New York", "USA", "Active", QDate::currentDate().addDays(-30), 15000.0, {"VIP", "Tech"}},
            {2, "Sarah Johnson", "sarah.j@company.com", "+1-555-0124", "Design Studio", "456 Oak Ave", "Los Angeles", "USA", "Active", QDate::currentDate().addDays(-15), 8500.0, {"Design", "Creative"}},
            {3, "Michael Brown", "m.brown@business.com", "+1-555-0125", "Business Solutions", "789 Pine St", "Chicago", "USA", "Prospect", QDate::currentDate().addDays(-7), 0.0, {"Prospect"}},
            {4, "Emily Davis", "emily.davis@startup.com", "+1-555-0126", "Startup Inc", "321 Elm St", "Austin", "USA", "Active", QDate::currentDate().addDays(-45), 22000.0, {"Startup", "Tech"}},
            {5, "David Wilson", "d.wilson@enterprise.com", "+1-555-0127", "Enterprise Corp", "654 Maple Dr", "Seattle", "USA", "Inactive", QDate::currentDate().addDays(-90), 45000.0, {"Enterprise"}}
        };
    ...
    void addCustomerToTree(const Customer& customer) {
        ...
        auto* valueItem = new QStandardItem(QString("$%1").arg(customer.totalValue, 0, 'f', 2));
        ...
        m_customerModel->appendRow({nameItem, companyItem, statusItem, valueItem});
    }
    ...
    bool validateCustomerForm() {
        if (m_nameInput->text().isEmpty()) {
            FluentToastGlobal::showError("Validation Error", "Customer name is required.");
            m_nameInput->setFocus();
            return false;
        }
        
        if (m_emailInput->text().isEmpty()) {
            FluentToastGlobal::showError("Validation Error", "Email address is required.");
    ...
    void updateCustomerCount() {
        m_customerCountLabel->setText(QString("%1 customers").arg(m_customers.size()));
    }
    
    void connectSignals() {
        connect(m_customerTree, &QTreeView::clicked, this, &CustomerManagementSystem::onCustomerSelected);
        connect(m_saveButton, &FluentButton::clicked, this, &CustomerManagementSystem::saveCustomer);
        connect(m_exportButton, &FluentButton::clicked, this, &CustomerManagementSystem::exportCustomers);
        
        // Search with debouncing
        auto* searchTimer = new QTimer(this);
        searchTimer->setSingleShot(true);
        searchTimer->setInterval(300);
        connect(searchTimer, &QTimer::timeout, this, &CustomerManagementSystem::searchCustomers);
