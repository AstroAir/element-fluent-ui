...
public:
    ...
    FluentRTLAdaptationResult adaptWidget(QWidget* widget,
                                          bool recursive = true);
    FluentRTLAdaptationResult adaptLayout(QLayout* layout,
                                          bool recursive = true);
    ...
    // Text handling
    QString processRTLText(
        const QString& text,
        FluentRTLTextAlignment alignment = FluentRTLTextAlignment::Auto) const;
    QTextOption createRTLTextOption(
        FluentRTLTextAlignment alignment = FluentRTLTextAlignment::Auto);
    void configureTextDocumentForRTL(QTextDocument* document,
    ...
    double getOptimalLetterSpacing(const QFont& font,
                                   const QLocale& locale) const;
    double getOptimalWordSpacing(const QFont& font,
                                 const QLocale& locale) const;

    // Animation and transition adaptation
    QString adaptAnimationForRTL(const QString& animationName) const;
    QPropertyAnimation* createRTLAnimation(QObject* target,
    ...
    QString wrapWithBidiIsolation(const QString& text,
                                  FluentTextDirection direction) const;
    QString wrapWithBidiOverride(const QString& text,
                                 FluentTextDirection direction) const;
    ...
    int getTextWidth(const QString& text, const QFont& font,
                     FluentTextDirection direction) const;
    QSize getTextSize(const QString& text, const QFont& font,
                      FluentTextDirection direction) const;
    QRect getTextBoundingRect(const QString& text, const QFont& font,
                              const QRect& rect,
                              FluentTextDirection direction) const;
...
signals:
    ...
    void textDirectionDetected(const QString& text,
                               FluentTextDirection direction);
...
private:
    ...
    // Caching
    QString generateCacheKey(const QString& operation,
    ...
    QList<QPair<int, FluentTextDirection>> analyzeBidiRuns(
...
#define FLUENT_RTL_MIRROR_RECT(rect, container)                          \
    FluentQt::Core::FluentRTLSupportManager::instance().mirrorRect(rect, \
                                                                   container)

#define FLUENT_RTL_DETECT_DIRECTION(text)                                    \
    FluentQt::Core::FluentRTLSupportManager::instance().detectTextDirection( \
        text)
