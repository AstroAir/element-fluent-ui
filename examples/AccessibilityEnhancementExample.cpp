...
public:
    explicit AccessibilityDemoWidget(QWidget* parent = nullptr)
        : QWidget(parent) {
        setupUI();
        setupAccessibility();
        connectSignals();
...
private slots:
    void runAccessibilityCheck() {
        ...
        m_resultsText->append(
            QString("Found %1 accessibility issues:").arg(issues.size()));
        ...
        for (const auto& issue : issues) {
            ...
            m_resultsText->append(
                QString("  Suggestion: %1").arg(issue.suggestion));
            m_resultsText->append(
                QString("  WCAG: %1").arg(issue.wcagCriterion));
            m_resultsText->append("");
        }

        // Announce results to screen reader
        auto& screenReader =
            Accessibility::FluentScreenReaderManager::instance();
        screenReader.announce(
            QString("Accessibility check completed. Found %1 issues.")
                .arg(issues.size()),
    ...
    void applyAccessibilityFixes() {
        ...
        m_resultsText->append(
            QString("Applied %1 automatic fixes.").arg(fixedCount));

        // Announce to screen reader
        auto& screenReader =
            Accessibility::FluentScreenReaderManager::instance();
        screenReader.announce(
            QString("Applied %1 accessibility fixes.").arg(fixedCount),
    ...
    void toggleHighContrast() {
        ...
        m_highContrastBtn->setText(enabled ? "Disable High Contrast"
        ...
        if (enabled) {
            setStyleSheet(
                ...
                "QPushButton { background-color: #333; border: 2px solid "
                "white; } "
    ...
    void toggleScreenReaderMode() {
        ...
        m_screenReaderBtn->setText(enabled ? "Disable Screen Reader Mode"
                                           : "Enable Screen Reader Mode");

        if (enabled) {
            screenReader.announce(
                "Screen reader mode enabled. All interactions will be "
                "announced.",
                Accessibility::FluentAnnouncementPriority::High);
        }
    }
...
private:
    ...
    QWidget* createFormPanel() {
        ...
        m_newsletterCheck->setAccessibleDescription(
            "Receive monthly updates and news");
        ...
        m_ageSlider->setAccessibleDescription(
        ...
        m_commentsEdit->setAccessibleDescription(
        ...
        m_submitBtn->setAccessibleDescription(
            "Submit the form with entered information");
    ...
    QWidget* createControlPanel() {
        ...
        checkBtn->setAccessibleDescription(
            "Perform WCAG 2.1 compliance check on the form");
        connect(checkBtn, &QPushButton::clicked, this,
                &AccessibilityDemoWidget::runAccessibilityCheck);

        // Auto-fix button
        auto* fixBtn = new QPushButton("Apply Automatic Fixes", this);
        fixBtn->setAccessibleDescription(
            "Automatically fix accessibility issues where possible");
        connect(fixBtn, &QPushButton::clicked, this,
                &AccessibilityDemoWidget::applyAccessibilityFixes);

        // High contrast toggle
        m_highContrastBtn = new QPushButton("Enable High Contrast", this);
        m_highContrastBtn->setAccessibleDescription(
            "Toggle high contrast mode for better visibility");
        connect(m_highContrastBtn, &QPushButton::clicked, this,
                &AccessibilityDemoWidget::toggleHighContrast);

        // Screen reader mode toggle
        m_screenReaderBtn = new QPushButton("Enable Screen Reader Mode", this);
        m_screenReaderBtn->setAccessibleDescription(
            "Toggle screen reader announcements");
        connect(m_screenReaderBtn, &QPushButton::clicked, this,
                &AccessibilityDemoWidget::toggleScreenReaderMode);
        ...
        m_resultsText->setAccessibleDescription(
            "Results from accessibility compliance checking");
    ...
    void setupAccessibility() {
        ...
        screenReader.setLandmark(this, Accessibility::FluentAriaRole::Main,
                                 "Main Content");

        // Set form as form landmark
        auto* formPanel = findChild<QGroupBox*>("FormPanel");
        if (formPanel) {
            screenReader.setLandmark(
                formPanel, Accessibility::FluentAriaRole::Form, "Contact Form");
        }

        // Set up live regions for dynamic content
        screenReader.setLiveRegion(
            m_resultsText, Accessibility::FluentLiveRegionType::Polite, true);

        // Set up keyboard navigation
        auto& keyboard =
            Accessibility::FluentKeyboardNavigationManager::instance();
        ...
        // Add skip links
        keyboard.addSkipLink("Skip to form", findChild<QGroupBox*>("FormPanel"),
                             QKeySequence("Alt+F"));
        keyboard.addSkipLink("Skip to results", m_resultsText,
                             QKeySequence("Alt+R"));
    }
    ...
    void connectSignals() {
        ...
        connect(m_nameEdit, &QLineEdit::textChanged, this,
                &AccessibilityDemoWidget::onFormFieldChanged);
        connect(m_emailEdit, &QLineEdit::textChanged, this,
                &AccessibilityDemoWidget::onFormFieldChanged);
        connect(m_countryCombo,
                QOverload<int>::of(&QComboBox::currentIndexChanged), this,
                &AccessibilityDemoWidget::onFormFieldChanged);
        connect(m_newsletterCheck, &QCheckBox::toggled, this,
                &AccessibilityDemoWidget::onFormFieldChanged);
        ...
        connect(
            &manager,
            &Accessibility::FluentAccessibilityManager::accessibilityIssueFound,
...
public:
    explicit AccessibilityMainWindow(QWidget* parent = nullptr)
        : QMainWindow(parent) {
        setupUI();
        setupAccessibility();
    }
...
private:
    ...
    void setupAccessibility() {
        ...
        auto& screenReader =
            Accessibility::FluentScreenReaderManager::instance();
        screenReader.setAriaRole(this,
                                 Accessibility::FluentAriaRole::Application);
        screenReader.setAriaLabel(this,
                                  "FluentQt Accessibility Demo Application");

        // Announce application startup
        QTimer::singleShot(1000, [&screenReader]() {
            screenReader.announce(
                "FluentQt Accessibility Demo loaded. Use Tab to navigate, F1 "
                "for help.",
                Accessibility::FluentAnnouncementPriority::High);
        });
    }
