...
private:
    ...
    double calculateOperationsPerSecond(int operations,
...
void FluentPerformanceBenchmark::initTestCase() {
    ...
    m_benchmarkSuite =
        new FluentBenchmarkSuite("FluentQt Performance Tests", this);
    ...
    qDebug() << "Initial frame rate:"
             << m_performanceMonitor->currentFrameRate();
    qDebug() << "Initial memory usage:"
             << m_performanceMonitor->currentMemoryUsage() / (1024 * 1024)
             << "MB";
}
...
void FluentPerformanceBenchmark::cleanupTestCase() {
    ...
    for (const auto& result : results) {
        qDebug() << result.testName << ":" << result.averageTime.count()
                 << "ms avg," << result.operationsPerSecond << "ops/sec";
    }
}
...
void FluentPerformanceBenchmark::benchmarkButtonCreation() {
    ...
    QVERIFY2(m_performanceMonitor->currentFrameRate() > 30.0,
...
void FluentPerformanceBenchmark::benchmarkTextInputCreation() {
    ...
    QBENCHMARK {
        ...
        auto elapsed = std::chrono::milliseconds(timer.elapsed());
        qDebug() << "Created" << count << "text inputs in" << elapsed.count()
                 << "ms";
    }
}
...
void FluentPerformanceBenchmark::benchmarkMemoryPoolAllocation() {
    ...
    QBENCHMARK {
        ...
        auto elapsed = std::chrono::milliseconds(timer.elapsed());
        qDebug() << "Memory pool: " << iterations << "allocations in"
                 << elapsed.count() << "ms";
    }
}
...
void FluentPerformanceBenchmark::benchmarkStandardAllocation() {
    ...
    QBENCHMARK {
        ...
        for (int i = 0; i < iterations; ++i) {
            auto* button =
                new FluentButton(QString("Button %1").arg(i), nullptr);
            buttons.push_back(button);
        }
        ...
        auto elapsed = std::chrono::milliseconds(timer.elapsed());
        qDebug() << "Standard allocation: " << iterations << "allocations in"
                 << elapsed.count() << "ms";
    }
}
...
void FluentPerformanceBenchmark::benchmarkComponentRendering() {
    ...
    QBENCHMARK {
        ...
        auto elapsed = std::chrono::milliseconds(timer.elapsed());
        qDebug() << "Rendered" << m_testComponents.size() << "components in"
                 << elapsed.count() << "ms";
    }
...
void FluentPerformanceBenchmark::benchmarkAnimationPerformance() {
    ...
    for (int i = 0; i < buttonCount; ++i) {
        auto* button =
            new FluentButton(QString("Animated %1").arg(i), m_testWidget);
    ...
    QBENCHMARK {
        ...
        for (auto* button : buttons) {
            QHoverEvent hoverEvent(QEvent::HoverEnter, QPointF(50, 15),
                                   QPointF(50, 15));
        ...
        auto elapsed = std::chrono::milliseconds(timer.elapsed());
        qDebug() << "Animated" << buttonCount << "buttons in" << elapsed.count()
                 << "ms";
    }
...
void FluentPerformanceBenchmark::stressTestManyComponents() {
    ...
    qDebug() << "  Created" << componentCount << "components in"
             << elapsed.count() << "ms";
    qDebug() << "  Memory usage:"
             << (finalMemory - initialMemory) / (1024 * 1024) << "MB";
    qDebug() << "  Frame rate: " << initialFrameRate << "->" << finalFrameRate
             << "FPS";

    // Performance assertions
    QVERIFY2(finalFrameRate > 20.0,
             "Frame rate should remain above 20 FPS under stress");
    QVERIFY2((finalMemory - initialMemory) < 100 * 1024 * 1024,
...
void FluentPerformanceBenchmark::benchmarkComplexUICreation() {
    QBENCHMARK {
        ...
        for (int i = 0; i < 5; ++i) {
            auto* button =
                new FluentButton(QString("Header %1").arg(i), m_testWidget);
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            m_testComponents.insert(
                m_testComponents.end(),
                {card, cardButton, cardInput, cardProgress});
        }
...
void FluentPerformanceBenchmark::createTestComponents(int count) {
    for (int i = 0; i < count; ++i) {
        auto* button =
            new FluentButton(QString("Test %1").arg(i), m_testWidget);
...
void FluentPerformanceBenchmark::measureMemoryUsage(const QString& testName) {
    ...
    qDebug() << testName << "memory usage:" << currentUsage / (1024 * 1024)
             << "MB";
}

double FluentPerformanceBenchmark::calculateOperationsPerSecond(
    int operations, std::chrono::milliseconds duration) {
    if (duration.count() == 0)
        return 0.0;
