...
private slots:
    void showBasicToast() {
        FluentToastType type = static_cast<FluentToastType>(
            m_toastTypeSelect->currentData().toInt());
        QString title = m_toastTitleEdit->text().isEmpty()
                            ? "Sample Toast"
                            : m_toastTitleEdit->text();
        QString message = m_toastMessageEdit->text().isEmpty()
                              ? "This is a sample toast message."
                              : m_toastMessageEdit->text();
        ...
        switch (type) {
            ...
            case FluentToastType::Custom:
                FluentToastGlobal::showCustom(QIcon(":/icons/star.png"), title,
                                              message);
                break;
        }
    }
    ...
    void showActionToast() {
        auto* toast = FluentToast::createInfo(
            "File Download", "Download completed successfully.");
        ...
        toast->addAction(
            "Show in Folder",
            [=]() {
                FluentToastGlobal::showInfo("Action",
    ...
    void showProgressToast() {
        auto* progressToast =
            FluentToast::createInfo("Processing", "Please wait...");
    ...
    void showPersistentToast() {
        auto* persistentToast = FluentToast::createWarning(
            "Important Notice", "This message requires your attention.");
        persistentToast->setPersistent(true);
        persistentToast->setClosable(true);

        persistentToast->addAction("Acknowledge", [=]() {
            FluentToastGlobal::showSuccess("Acknowledged",
    ...
    void onToastPositionChanged() {
        FluentToastPosition position = static_cast<FluentToastPosition>(
            m_positionSelect->currentData().toInt());
    ...
    void hideAllToasts() { FluentToastManager::instance().hideAll(); }

    void onCountryChanged() {
        QString country = m_countrySelect->currentText();
        if (!country.isEmpty()) {
            FluentToastGlobal::showInfo(
                "Country Selected", QString("You selected: %1").arg(country));
        }
    }
    ...
    void onLanguagesChanged() {
        ...
        if (!languages.isEmpty()) {
            QString message =
                QString("Selected languages: %1").arg(languages.join(", "));
    ...
    void onSkillsChanged() {
        ...
        m_selectedSkillsLabel->setText(
            QString("Selected: %1").arg(skills.join(", ")));
    }
    ...
    void addCustomSkill() {
        ...
        if (!skill.isEmpty() && m_skillsSelect->findText(skill) == -1) {
            ...
            FluentToastGlobal::showSuccess(
                "Skill Added",
                QString("Added '%1' to skills list.").arg(skill));
        }
    }
...
private:
    void setupUI() {
        ...
        m_toastTypeSelect->addItem("Info",
                                   static_cast<int>(FluentToastType::Info));
        m_toastTypeSelect->addItem("Success",
                                   static_cast<int>(FluentToastType::Success));
        m_toastTypeSelect->addItem("Warning",
                                   static_cast<int>(FluentToastType::Warning));
        m_toastTypeSelect->addItem("Error",
                                   static_cast<int>(FluentToastType::Error));
        m_toastTypeSelect->addItem("Custom",
                                   static_cast<int>(FluentToastType::Custom));
        ...
        m_positionSelect->addItem(
            "Top Left", static_cast<int>(FluentToastPosition::TopLeft));
        m_positionSelect->addItem(
            "Top Center", static_cast<int>(FluentToastPosition::TopCenter));
        m_positionSelect->addItem(
            "Top Right", static_cast<int>(FluentToastPosition::TopRight));
        m_positionSelect->addItem(
            "Bottom Left", static_cast<int>(FluentToastPosition::BottomLeft));
        m_positionSelect->addItem(
            "Bottom Center",
            static_cast<int>(FluentToastPosition::BottomCenter));
        m_positionSelect->addItem(
            "Bottom Right", static_cast<int>(FluentToastPosition::BottomRight));
        m_positionSelect->addItem(
            "Center", static_cast<int>(FluentToastPosition::Center));
        ...
        // Toast demo buttons
        auto* basicToastBtn =
            FluentButton::createPrimaryButton("Show Basic Toast");
        ...
        m_countrySelect->addItems({"United States", "Canada", "United Kingdom",
        ...
        m_languageSelect->addItems({"English", "Spanish", "French", "German",
        ...
        m_skillsSelect->addItems({"C++", "Python", "JavaScript", "Java", "C#",
    ...
    void setupConnections() {
        ...
        connect(m_positionSelect,
                QOverload<int>::of(&FluentSelect::currentIndexChanged), this,
                &ToastAndSelectDemo::onToastPositionChanged);
